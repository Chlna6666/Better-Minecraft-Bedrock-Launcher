...
//! [`ScanReport`] where possible so a single unknown record does not stop
//! a full-world scan.

use crate::chunk::{
    ActorUid, BedrockDbKey, ChunkPos, ChunkRecord, ChunkRecordTag, ChunkVersion, GlobalRecordKind,
    LegacyTerrain, SubChunk, SubChunkDecodeMode, SubChunkFormat,
};
use crate::error::{BedrockWorldError, Result as WorldResult};
use crate::level_dat::LevelDatDocument;
use crate::map_item::{KnownFields, MapItemId, Pixels, SavedData};
...
use crate::village::{Entry, VillageKey};
use bytes::Bytes;
...
pub struct ScanReport {
    ...
    /// Number of Bedrock map items decoded.
...
#[derive(Debug, Clone, PartialEq)]
/// Structured snapshot of one chunk and its inspection report.
pub struct Chunk {
...
pub enum ChunkValue {
    ...
    HardcodedSpawnAreas(Vec<HardcodedSpawnArea>),
    /// Raw bytes preserved because the payload was not decoded.
    Raw(Bytes),
}
...
impl Biome3d {
    ...
    pub fn new(height_map: Vec<i16>, storages: Vec<BiomeStorage>) -> WorldResult<Self> {
        HeightMap2d::new(height_map.clone())?;
        Ok(Self {
            height_map,
            storages,
            ...
            .map(|record| {
                *report
                ...
                ChunkEntry {
                    ...
                    value: parse_chunk_record_value(
                        &record.key,
                        &record.value,
                        &mut report,
                        options,
...
fn should_parse_key(key: &BedrockDbKey, categories: ScanCategories) -> bool {
    match key {
...
fn parse_entry_value(
    ...
    match key {
        BedrockDbKey::Chunk(chunk_key) => ScanValue::Chunk(ChunkEntry {
            key: chunk_key.clone(),
...
fn parse_legacy_terrain(value: &Bytes, report: &mut ScanReport) -> ChunkValue {
...
fn parse_actor_digest_value(
    ...
    for actor_id_bytes in value.chunks_exact(8) {
        ...
        match parse_actor_value(actor_value, report) {
            ScanValue::ActorEntities(mut parsed_entities) => {
                entities.append(&mut parsed_entities);
            }
            ScanValue::Raw(_)
            | ScanValue::Chunk(_)
            | ScanValue::Player(_)
            | ScanValue::ActorDigest(_)
            | ScanValue::MapData(_)
            | ScanValue::VillageData(_)
            | ScanValue::GlobalData(_)
            | ScanValue::NbtRoots(_) => {}
        }
    }
    report.actor_digest_missing_count += missing_actor_count;
    ScanValue::ActorDigest(ActorDigest {
        pos,
        actor_ids,
        entities,
        missing_actor_count,
...
fn raw_db_value(value: &Bytes, options: ScanOptions) -> ScanValue {
    if options.retention.retains_raw() {
        ScanValue::Raw(value.clone())
    } else {
        ScanValue::Raw(Bytes::new())
    }
}

fn raw_chunk_value(value: &Bytes, options: ScanOptions) -> ChunkValue {
    if options.retention.retains_raw() {
        ChunkValue::Raw(value.clone())
    } else {
        ChunkValue::Raw(Bytes::new())
    }
}

fn parse_map_value(id: &str, value: &Bytes, report: &mut ScanReport) -> ScanValue {
    let roots = match parse_consecutive_root_nbt(value) {
        ...
        Err(error) => {
            ...
            report.raw_entry_count += 1;
            return ScanValue::Raw(value.clone());
        }
    };
    report.map_item_count += 1;
    let known_fields = map_known_fields(&roots);
    let pixels = map_pixels(&roots);
    ScanValue::MapData(SavedData {
        id: MapItemId::unchecked(id.to_string()),
        roots,
        known_fields,
        pixels,
        raw: value.clone(),
...
/// Decodes a Bedrock `map_<id>` item value.
pub fn decode_map_item(id: MapItemId, value: Bytes) -> WorldResult<SavedData> {
...
/// Encodes a Bedrock `map_<id>` item value.
pub fn encode_map_item(item: &SavedData) -> WorldResult<Bytes> {
    encode_consecutive_roots(&item.roots)
}
...
/// Encodes a known Bedrock global value.
pub fn encode_global(global: &Global) -> WorldResult<Bytes> {
    encode_consecutive_roots(&global.roots)
}

/// Decodes actor UIDs stored in a `digp` value.
pub fn decode_actor_ids(value: &[u8]) -> WorldResult<Vec<ActorUid>> {
    if !value.len().is_multiple_of(8) {
...
/// Encodes actor UIDs for a `digp` value.
pub fn encode_actor_ids(actor_ids: &[ActorUid]) -> Bytes {
...
fn map_known_fields(roots: &[NbtTag]) -> KnownFields {
    let Some(root) = roots.first().and_then(compound) else {
        return KnownFields::default();
    };
    KnownFields {
...
fn map_pixels(roots: &[NbtTag]) -> Option<Pixels> {
...
pub(crate) fn parse_actor_value(value: &Bytes, report: &mut ScanReport) -> ScanValue {
    match parse_consecutive_root_nbt(value) {
        Ok(tags) => {
            let entities = tags
                .into_iter()
            ...
            ScanValue::ActorEntities(entities)
        }
        Err(error) => {
            report
                .parse_errors
...
pub(crate) fn parse_legacy_data2d(value: &[u8]) -> Result<BiomeData, String> {
    if value.len() < 768 {
...
pub(crate) fn parse_data2d_legacy(value: &[u8]) -> Result<BiomeData, String> {
...
fn encode_biome_storage(storage: &BiomeStorage) -> WorldResult<Vec<u8>> {
...
fn read_hardcoded_spawn_areas(value: &[u8]) -> Result<Vec<HardcodedSpawnArea>, String> {
...
pub(crate) fn parse_block_entities_from_value(
    ...
    match parse_block_entities(value, report) {
        ChunkValue::BlockEntities(block_entities) => block_entities,
        _ => Vec::new(),
    }
}

fn parse_pending_ticks(value: &Bytes, report: &mut ScanReport) -> ChunkValue {
    match parse_consecutive_root_nbt(value) {
        Ok(tags) => ChunkValue::PendingTicks(tags),
        Err(error) => {
            report
                .parse_errors
...
fn parse_entity_from_nbt(nbt: NbtTag, report: &mut ScanReport) -> Actor {
    let items = collect_item_stacks(&nbt);
    report.item_count += items.len();
    let root = compound(&nbt);
    Actor {
...
fn parse_block_entity_from_nbt(nbt: NbtTag, report: &mut ScanReport) -> BlockEntity {
    let items = collect_item_stacks(&nbt);
    report.item_count += items.len();
    let root = compound(&nbt);
    BlockEntity {
...
mod tests {
    ...
    fn entity_extracts_identifier_position_and_items() {
        ...
        let ScanValue::ActorEntities(entities) = value else {
    ...
    fn block_entity_extracts_container_items() {
        ...
        let ChunkValue::BlockEntities(block_entities) = value else {
    ...
    fn hsa_records_roundtrip_reference_binary_layout() {
        let areas = vec![HardcodedSpawnArea {
            kind: HardcodedSpawnAreaKind::PillagerOutpost,
            min: [1, 2, 3],
            max: [4, 5, 6],
        }];

        let bytes = encode_hardcoded_spawn_areas(&areas).expect("encode hsa");
        let decoded = decode_hardcoded_spawn_areas(&bytes).expect("decode hsa");
    ...
    fn malformed_known_chunk_records_are_reported_as_parse_errors() {
        ...
        let parsed = Chunk::new(pos, &records, ScanOptions::full());
        ...
        assert!(
            parsed
                .records
                .iter()
                .all(|record| matches!(record.value, ChunkValue::Raw(_)))
        );
    }
    ...
    fn malformed_map_nbt_is_raw_instead_of_an_empty_typed_map() {
        ...
        assert!(matches!(parsed, ScanValue::Raw(_)));
        assert_eq!(report.map_item_count, 0);
