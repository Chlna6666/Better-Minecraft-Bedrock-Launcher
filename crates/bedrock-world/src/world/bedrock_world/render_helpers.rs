...
pub(super) fn check_cancelled(options: &WorldScanOptions) -> Result<()> {
    if options
        .cancel
        .as_ref()
...
pub(super) fn emit_progress(options: &WorldScanOptions, entries_seen: usize) {
    if let Some(progress) = &options.progress {
        progress.emit(WorldScanProgress { entries_seen });
    }
}

pub(super) fn check_render_load_cancelled(options: &ChunkLoadOptions) -> Result<()> {
    if options
        .cancel
        .as_ref()
        .is_some_and(CancelFlag::is_cancelled)
...
pub(super) fn emit_render_load_progress(options: &ChunkLoadOptions, completed_chunks: usize) {
...
pub(super) fn sort_render_chunk_positions(positions: &mut [ChunkPos], priority: ChunkLoadPriority) {
    match priority {
...
pub(super) fn push_render_record_request(
    keys: &mut Vec<Bytes>,
...
pub(super) fn apply_render_record_values(
...
pub(super) fn planned_render_subchunk_ys(
    pos: ChunkPos,
...
pub(super) fn request_needs_biome_record(options: &ChunkLoadOptions) -> bool {
    let request = &options.data_request;
    request.height_map || !matches!(request.biome, BiomeDataRequirement::None)
}

pub(super) fn request_needs_legacy_terrain(options: &ChunkLoadOptions) -> bool {
...
pub(super) fn request_needs_legacy_terrain_fallback(options: &ChunkLoadOptions) -> bool {
    !request_needs_legacy_terrain(options) && !options.data_request.subchunks.is_empty()
}

pub(super) fn request_loads_block_entities(options: &ChunkLoadOptions) -> bool {
    options.data_request.block_entities
}

pub(super) fn request_builds_column_samples(options: &ChunkLoadOptions) -> bool {
    options
        .data_request
        .subchunks
        .iter()
...
pub(super) fn request_uses_hint_surface_subchunks(options: &ChunkLoadOptions) -> bool {
...
pub(super) fn exact_surface_full_request(options: &mut ChunkLoadOptions) {
...
pub(super) fn insert_render_biome_storages(
...
pub(super) fn parse_render_biome_record(
...
pub(super) fn render_height_map_from_biome_data(
    pos: ChunkPos,
...
pub(super) fn normalize_biome_height(
    pos: ChunkPos,
...
pub(super) fn legacy_height_map_from_raw(
...
pub(super) fn render_height_map_from_legacy_terrain(
...
pub(super) fn render_biomes_from_legacy_terrain(
    terrain: &LegacyTerrain,
...
pub(super) fn render_biome_colors_from_legacy_terrain(
    terrain: &LegacyTerrain,
) -> [[Option<u32>; 16]; 16] {
...
pub(super) fn build_terrain_column_samples(
    pos: ChunkPos,
    version: crate::ChunkVersion,
...
#[allow(clippy::too_many_arguments)]
pub(super) fn scan_terrain_surface_state(
    local_x: u8,
    local_z: u8,
    y: i32,
    height: i16,
...
pub(super) fn legacy_terrain_block_state_at(
    local_x: u8,
    y: i32,
    local_z: u8,
    subchunks: &BTreeMap<i8, SubChunk>,
...
pub(super) fn render_biome_id_at(
    local_x: u8,
    local_z: u8,
    y: i32,
    render_biomes: &BTreeMap<i32, ParsedBiomeStorage>,
...
pub(super) fn render_chunk_from_raw(
    raw: RawChunkData,
    options: &ChunkLoadOptions,
...
pub(super) fn render_load_stats(
    chunks: &[ChunkData],
...
pub(super) fn log_render_load_complete(stats: &ChunkLoadStats) {
    log::debug!(
...
pub(super) fn to_storage_read_options(options: &WorldScanOptions) -> StorageReadOptions {
    StorageReadOptions {
...
pub(super) fn to_render_storage_read_options(options: &ChunkLoadOptions) -> StorageReadOptions {
    StorageReadOptions {
        threading: match options.threading {
            WorldThreadingOptions::Auto => StorageThreadingOptions::Auto,
            WorldThreadingOptions::Fixed(threads) => StorageThreadingOptions::Fixed(threads),
...
pub(super) fn chunk_record_prefix(pos: ChunkPos) -> Bytes {
...
pub(super) fn validate_render_region(region: WorldChunkQueryRegion) -> Result<()> {
...
pub(super) fn render_block_entity_from_nbt(nbt: NbtTag) -> ChunkBlockEntity {
    let root = match &nbt {
...
pub(super) fn nbt_string_field<'a>(
...
pub(super) fn nbt_int_field(root: &indexmap::IndexMap<String, NbtTag>, key: &str) -> Option<i32> {
    match root.get(key) {
