...
//!
//! Terrain remains sourced from `chunks.dat`; the `Entities` and `TileEntities` lists from
//! `entities.dat` are exposed as virtual legacy `Entity` and `BlockEntity` chunk records so normal
//! BedrockWorld query APIs can inspect the complete old world without caller-side file plumbing.
