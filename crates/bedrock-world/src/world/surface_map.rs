...
use crate::parsed::{
    ParsedBiomeData, ParsedBiomeStorage, parse_data2d_legacy, parse_data3d, parse_legacy_data2d,
};
    ...
    S: WorldStorageHandle,
{
    ...
    pub fn query_surface_map_many_blocking(
        ...
        let values = self
            .storage()
            .get_many_ordered_with_control(keys.keys(), storage_options)?;
        ...
        if !fallback_positions.is_empty() {
            ...
            let (chunks, stats) =
                self.query_chunk_data_with_stats_blocking(fallback_positions, load_options)?;
...
fn decode_direct_surface_chunk(raw: RawSurfaceChunk) -> Result<SurfaceDecodeOutput> {
    ...
    for (y, value) in raw.subchunks {
        let subchunk = parse_subchunk_with_mode(y, value, SubChunkDecodeMode::SurfaceColumns)?;
