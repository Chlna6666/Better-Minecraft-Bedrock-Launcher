//! Query models, render request state, threading, cancellation, and progress types.
...
#[derive(Debug, Clone)]
pub(super) struct RawChunkData {
    pub(super) pos: ChunkPos,
    pub(super) biome_record: Option<(ChunkRecordTag, Bytes)>,
    pub(super) subchunks: BTreeMap<i8, Bytes>,
    pub(super) block_entities: Option<Bytes>,
    pub(super) legacy_terrain: Option<Bytes>,
}

#[derive(Debug, Clone, Copy, Default)]
#[allow(clippy::struct_field_names)]
pub(super) struct ChunkDecodeTiming {
    pub(super) biome_parse_us: u128,
    pub(super) subchunk_parse_us: u128,
    pub(super) surface_scan_us: u128,
    pub(super) block_entity_parse_us: u128,
}
...
pub(super) enum RenderRecordKind {
    LegacyTerrain,
    Data3D,
    Data2D,
    Data2DLegacy,
...
impl RenderRecordKind {
    pub(super) fn biome_tag(self) -> ChunkRecordTag {
        match self {
...
#[derive(Debug, Clone, Copy)]
pub(super) struct RenderRecordRequest {
    pub(super) chunk_index: usize,
    pub(super) kind: RenderRecordKind,
}
...
impl ChunkDataRequest {
    pub(super) fn block_entities_if(mut self, enabled: bool) -> Self {
        self.block_entities = enabled;
        self
    }

    pub(super) fn preferred_decode_mode(&self) -> SubChunkDecodeMode {
...
impl ChunkBounds {
    pub(super) fn from_first(pos: ChunkPos) -> Self {
        Self {
    ...
    pub(super) fn include(&mut self, pos: ChunkPos) {
...
pub(super) fn default_world_worker_budget() -> usize {
    let logical = std::thread::available_parallelism().map_or(1, usize::from);
    logical.div_ceil(2).clamp(2, 6)
}

pub(super) fn world_executor(worker_count: usize) -> Result<Arc<WorldExecutor>> {
...
pub struct CancelFlag(pub(super) Arc<AtomicBool>);
...
impl ProgressSink {
    ...
    pub(super) fn emit(&self, progress: WorldScanProgress) {
