...
fn player_listing_uses_key_only_prefix_scan() {
    let world = World::from_typed_storage("memory", KeyOnlyPlayerStorage, OpenOptions::default());

    assert_eq!(
        world.players().expect("list players"),
...
fn map_and_global_records_roundtrip_through_world_transactions() {
    ...
    let map_id = MapItemId::new("9").expect("map id");
    let map = SavedData {
        id: map_id.clone(),
    ...
    world.save_map_item(&map).expect("write map");
    let read_map = world
        .map_item(&map_id)
        .expect("read map")
        .expect("map exists");
...
fn hsa_and_block_entities_roundtrip_with_chunk_validation() {
    ...
    world
        .save_hardcoded_spawn_areas(pos, std::slice::from_ref(&area))
        .expect("write hsa");
    assert_eq!(
        world
...
fn actor_write_updates_digest_and_prefix_together() {
    ...
    world.put_actor(pos, &actor).expect("put actor");
    let digest = storage
        .get(&ActorDigestKey::new(pos).storage_key())
        .expect("get digest")
        .expect("digest exists");
    assert_eq!(
        decode_actor_ids(&digest).expect("parse digest"),
        vec![actor_uid]
    );
    assert!(
        storage
...
fn generic_memory_storage_matches_dynamic_storage_queries() {
    ...
    let generic_world =
        World::from_typed_storage("memory", storage.clone(), OpenOptions::default());
    ...
    assert_eq!(
        generic_world.players().expect("generic"),
        dynamic_world.players().expect("dynamic")
    );
    assert_eq!(
        generic_world
...
fn generic_leveldb_storage_matches_dynamic_storage_queries() {
    ...
    let generic_world = World::from_typed_storage(&temp, storage.clone(), OpenOptions::default());
    let dynamic_world = World::from_storage(
        &temp,
        Arc::new(storage) as Arc<dyn WorldStorage>,
        OpenOptions::default(),
...
fn transaction_respects_read_only_option() {
    ...
    let storage = Arc::new(MemoryStorage::new());
    let read_only_world = World::from_storage("memory", storage.clone(), OpenOptions::default());
    let mut transaction = read_only_world.transaction();
    transaction.put_raw(&key, Bytes::from_static(b"\x01"));
    ...
    let writable_world = World::from_storage(
        "memory",
        storage.clone(),
        OpenOptions {
            read_only: false,
    ...
    transaction.put_raw(&key, Bytes::from_static(b"\x02"));
...
fn transaction_replaces_chunk_records_and_typed_payloads_in_one_commit() {
    ...
    transaction.put_raw(&new_key, Bytes::from_static(b"\x02\0\0\0"));
    transaction
        .put_block_entities(pos, std::slice::from_ref(&block_entity))
...
fn render_chunk_exact_load_preserves_data2d_xz_height_and_biome_coordinates() {
    ...
    let chunk = world
        .query_chunk_data(pos, ChunkLoadOptions::default())
        .expect("load render chunk");
...
fn subchunk_layer_query_uses_block_y() {
    ...
    let subchunk = world
        .subchunk_layer(pos, -1, SubChunkDecodeMode::CountsOnly)
        .expect("query")
        .expect("subchunk");
...
fn surface_column_query_returns_top_block_and_water_context() {
    ...
    let column = world
        .surface_column(pos, 0, 0, SurfaceColumnOptions::default())
...
fn chunk_bounds_and_nearest_loaded_chunk_use_key_only_scan() {
    ...
    let bounds = world
        .discover_chunk_bounds(Dimension::Overworld, WorldScanOptions::default())
        .expect("bounds")
...
fn legacy_terrain_is_renderable_and_exact_batch_loaded() {
    ...
    let world = World::from_storage_with_format(
        "memory",
        storage,
        OpenOptions::default(),
...
fn legacy_terrain_exposes_biome_colors_without_transposing_columns() {
    ...
    assert_eq!(
        world
            .legacy_biome_color(pos, 15, 0)
            .expect("legacy biome color"),
        Some(0x0044_5566)
    );
    assert_eq!(
        world
            .legacy_biome_sample(pos, 15, 0)
