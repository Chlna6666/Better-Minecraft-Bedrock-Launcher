...
/// Writes player NBT to `level.dat.Player` without touching LevelDB player keys.
    ...
    if let (Some(source), Some(target)) = (player.game_version(), target.last_opened_with.as_ref()) {
        if source != target {
            return Err(BedrockWorldError::Validation(format!(
                "player belongs to Bedrock {source}, target level.dat reports {target}; select the target player data generation explicitly before writing"
            )));
        }
    }
