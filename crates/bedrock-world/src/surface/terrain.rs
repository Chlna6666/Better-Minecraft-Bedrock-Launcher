...
//! Terrain column sampling and legacy terrain interpretation.

use super::*;

pub(crate) fn validate_local_column(local_x: u8, local_z: u8) -> Result<()> {
...
pub(crate) fn insert_needed_surface_subchunks(
...
pub(crate) fn block_y_to_subchunk_y(y: i32) -> Result<i8> {
...
pub(crate) fn biome_storage_contains_y(storage: &BiomeStorage, y: i32) -> bool {
    storage
        .y
        .is_none_or(|start_y| (start_y..start_y + 16).contains(&y))
}

pub(crate) fn biome_storage_bucket_y(y: i32) -> i32 {
    y.div_euclid(16) * 16
}

pub(crate) fn biome_id_from_storage(
...
pub(crate) fn height_map_index(local_x: u8, local_z: u8) -> usize {
    usize::from(local_z) * 16 + usize::from(local_x)
}

pub(crate) fn column_index(local_x: u8, local_z: u8) -> Option<usize> {
    (local_x < 16 && local_z < 16).then_some(height_map_index(local_x, local_z))
}

pub(crate) fn raw_height_at(
    height_map: Option<&[[Option<i16>; 16]; 16]>,
    local_x: u8,
    local_z: u8,
) -> Option<i16> {
...
pub(crate) fn raw_height_mismatch_columns(chunk: &ChunkData) -> usize {
...
pub(crate) fn missing_surface_columns(chunk: &ChunkData) -> usize {
    chunk.column_samples.as_ref().map_or(0, |samples| {
        256usize.saturating_sub(samples.sampled_columns())
    })
}

pub(crate) fn needed_exact_surface_chunk_requires_full_reload(chunk: &ChunkData) -> Result<bool> {
    let Some(samples) = chunk.column_samples.as_ref() else {
        return Ok(false);
    };
...
pub(crate) fn legacy_world_block_state(id: u8, data: u8) -> BlockState {
...
pub(crate) fn legacy_world_block_name(id: u8, data: u8) -> String {
    let name = match id {
        0 => "minecraft:air",
        1 => match data & 0x7 {
            1 => "minecraft:granite",
...
pub(crate) fn legacy_world_wood_name(data: u8, suffix: &'static str) -> &'static str {
...
pub(crate) fn legacy_world_wool_name(data: u8) -> &'static str {
    match data & 0x0f {
