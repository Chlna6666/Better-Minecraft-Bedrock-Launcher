//! Bedrock saved-item identities across persisted format generations.
//!
...
use crate::block::{LegacyNumericBlockStateTable, LegacyNumericBlockUpgradeTable};
...
/// BlockState lookup data used to validate saved-item `Block` payloads.
///
/// The numeric table is the persisted Classic source data. The upgraded table is its
/// forward-verified reverse lookup; both are required to prove an exact historical round trip.
#[derive(Debug, Clone, Copy)]
pub struct SavedItemBlockStates<'a> {
    /// Classic numeric BlockState source table.
...
impl<'a> SavedItemBlockStates<'a> {
    /// Creates one paired BlockState lookup.
    #[must_use]
    pub const fn new(
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ClassicSavedItemMatch {
...
impl ClassicSavedItemMatch {
    /// Returns the historical pair only when the representation is unique.
    #[must_use]
    pub const fn unique(self) -> Option<ClassicSavedItemId> {
        match self {
...
#[derive(Debug, Clone)]
pub struct SavedItemHistory {
    catalog: AuthoritativeItemMigrationCatalog,
    classic_ids: Vec<(i32, String)>,
    classic_names: Vec<String>,
...
impl SavedItemHistory {
    ...
    pub fn from_sources(
        ...
        let catalog = AuthoritativeItemMigrationCatalog::from_sources(
            classic_item_id_map_json,
            item_to_block_1_12_json,
            &internal_sources,
        )?;
        Self::from_catalog_and_sources(catalog, classic_item_id_map_json, sources)
    }
    ...
    fn from_catalog_and_sources(
        ...
        let source_ids: BTreeMap<String, i32> = serde_json::from_str(classic_item_id_map_json)
            .map_err(|error| validation(format!("invalid Classic numeric item ID map: {error}")))?;
        let mut classic_names = source_ids
            .iter()
        ...
        let mut classic_ids = source_ids
            .into_iter()
            .filter_map(|(name, numeric_id)| (numeric_id != 0).then_some((numeric_id, name)))
            .collect::<Vec<_>>();
        classic_ids.sort_by(|left, right| left.0.cmp(&right.0).then_with(|| left.1.cmp(&right.1)));
    ...
    #[must_use]
    pub fn classic_item_name(&self, legacy: ClassicSavedItemId) -> Option<&str> {
    ...
    pub fn medieval_id_from_classic(
        &self,
        legacy: ClassicSavedItemId,
    ) -> Option<MedievalSavedItemId> {
        let name = self.classic_item_name(legacy)?;
    ...
    #[must_use]
    pub fn classic_block_id(&self, legacy: ClassicSavedItemId) -> Option<&str> {
        let item_name = self.classic_item_name(legacy)?;
    ...
    #[must_use]
    pub fn named_id(&self, legacy: ClassicSavedItemId) -> Option<NamedSavedItemId> {
    ...
    #[must_use]
    pub fn match_numeric(&self, target: &NamedSavedItemId) -> ClassicSavedItemMatch {
        let target_already_present = self
            .remapped_source_metas
            .binary_search(&target.meta)
            .is_ok();
        let mut first = None::<ClassicSavedItemId>;

        for (numeric_id, historical_name) in &self.classic_ids {
            ...
            while index < self.remapped_source_metas.len() || !target_emitted {
                ...
                let candidate = ClassicSavedItemId {
                ...
                match first {
                    ...
                    Some(previous) => {
                        return ClassicSavedItemMatch::Ambiguous {
                            first: previous,
                            second: candidate,
                        };
                    }
    ...
    pub fn match_medieval(&self, target: &NamedSavedItemId) -> MedievalSavedItemMatch {
        ...
        for historical_name in &self.classic_names {
            let mut index = 0usize;
            let mut target_emitted = target_already_present;
            while index < self.remapped_source_metas.len() || !target_emitted {
                let meta = next_candidate_meta(
    ...
    #[must_use]
    pub fn classic_id_count(&self) -> usize {
        self.classic_ids.len()
    }
...
pub fn load_pinned_saved_item_history_from_dir(
    ...
    let sources = schema_json
        ...
        .collect::<Vec<_>>();
    SavedItemHistory::from_catalog_and_sources(catalog, &classic_json, &sources)
}
...
mod tests {
    ...
    fn medieval_candidate_uses_1_6_endpoint_name() {
        ...
        assert_eq!(
            table.medieval_id_from_classic(ClassicSavedItemId {
                numeric_id: 421,
                meta: 0,
            }),
            Some(MedievalSavedItemId {
    ...
    fn medieval_endpoint_aliases_are_deduplicated() {
        ...
        assert!(matches!(
            ...
            ClassicSavedItemMatch::Ambiguous { .. }
        ));
    }
    ...
    fn reverse_match_does_not_generate_numeric_air_id_zero() {
        let table =
            SavedItemHistory::from_sources(r#"{"minecraft:air":0,"minecraft:stone":1}"#, "{}", &[])
                .unwrap();
        assert_eq!(
            table.match_numeric(&NamedSavedItemId {
    ...
    fn block_mapping_is_resolved_at_1_12_endpoint() {
        ...
        .unwrap();
        let classic = ClassicSavedItemId {
            numeric_id: 5,
            meta: 3,
        };
        ...
        assert_eq!(
            table.classic_block_id(classic),
