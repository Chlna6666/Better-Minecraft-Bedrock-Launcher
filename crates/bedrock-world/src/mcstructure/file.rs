...
impl McStructureFile {
        ...
        S: StorageBackend,
    {
        ...
        let affected_chunks =
            Self::write_placed_chunks(world, placements, block_entities, &mut progress)?;
        ...
        S: StorageBackend,
    {
        ...
        for (index, (chunk, subchunks)) in placements.into_iter().enumerate() {
            ...
            for (subchunk_y, subchunk) in updated_subchunks {
                ...
                active_transaction
                    .put_raw(&ChunkKey::subchunk(chunk, subchunk_y), Bytes::from(bytes));
            }

            active_transaction.put_raw(
            ...
            if let Some(structure_entities) = block_entities.remove(&chunk) {
                ...
                if !roots.is_empty() {
                    ...
                    active_transaction
                        .put_raw(&ChunkKey::new(chunk, ChunkRecordTag::BlockEntity), value);
                }
            }
...
impl EntrySubchunkBuilder {
    ...
    fn from_chunk(chunk: &crate::LevelChunk, y: i8) -> Result<Self> {
...
fn update_height_map_from_subchunks(
    ...
    existing_chunk: &crate::LevelChunk,
