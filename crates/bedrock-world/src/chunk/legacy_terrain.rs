    ...
    S: StorageBackend,
{
    ...
    pub fn combine_legacy_terrain(&self) -> Result<LegacyTerrainCombineReport> {
...
fn commit_legacy_terrain_batch<S>(world: &World<S>, batch: &StorageBatch) -> Result<()>
where
    S: StorageBackend,
{
    if batch.is_empty() {
...
mod tests {
    ...
    fn direct_block_query_falls_back_to_legacy_terrain() {
        ...
        storage
            ...
            .unwrap();
        let world = World::from_typed_storage("memory", storage, OpenOptions::default());
