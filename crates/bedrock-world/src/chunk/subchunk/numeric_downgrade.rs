...
pub struct NumericSubChunkDowngradeReport {
...
impl NumericSubChunkDowngradeReport {
...
pub(crate) fn stage_subchunk_downgrade(
    storage: &dyn WorldStorage,
    target: SubChunkVersion,
    numeric: &LegacyNumericBlockUpgradeTable,
) -> Result<(StorageBatch, NumericSubChunkDowngradeReport)> {
    ...
    let mut report = NumericSubChunkDowngradeReport::new(target);
...
fn resolve_primary_palette(
    ...
    numeric: &LegacyNumericBlockUpgradeTable,
    report: &mut NumericSubChunkDowngradeReport,
) -> Result<Vec<(u8, u8)>> {
