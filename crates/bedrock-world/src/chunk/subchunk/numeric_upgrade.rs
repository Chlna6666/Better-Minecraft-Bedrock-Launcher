...
pub struct NumericSubChunkUpgradeReport {
...
impl NumericSubChunkUpgradeReport {
...
pub(crate) fn stage_numeric_subchunk_upgrade(
...
) -> Result<(StorageBatch, NumericSubChunkUpgradeReport)> {
    if !matches!(
        target,
    ...
    let mut report = NumericSubChunkUpgradeReport::new(target, target_palette);
    ...
    storage.for_each_entry(StorageReadOptions::default(), &mut |raw_key, value| {
        ...
        let encoded = target_subchunk.encode(target).map_err(|error| {
...
fn index_block_extra_data(
    storage: &dyn WorldStorage,
    report: &mut NumericSubChunkUpgradeReport,
...
mod tests {
    ...
    fn legacy_v7_numeric_blocks_upgrade_to_target_palette_before_commit() {
        ...
        let target_version = catalog.output_version().raw();
        let (batch, report) = stage_numeric_subchunk_upgrade(
            &storage,
            SubChunkVersion::V8,
            &numeric_table(),
            &catalog,
