...
use crate::chunk::{
    NumericSubChunkUpgradeReport, SubChunkUpgradeWriteReport, SubChunkVersion,
    stage_numeric_subchunk_upgrade, stage_paletted_subchunks_for_upgrade,
};
...
pub struct SubChunkUpgradeReport {
    ...
    pub legacy: NumericSubChunkUpgradeReport,
    ...
    S: StorageBackend,
{
    ...
    pub fn upgrade_bedrock_subchunks(
        ...
        let (legacy_batch, legacy) = stage_numeric_subchunk_upgrade(
            self.storage(),
            target_version,
...
fn validate_upgrade_source<S>(world: &World<S>, target: &GameVersion) -> Result<()>
where
    S: StorageBackend,
{
...
fn commit_subchunk_upgrade<S>(world: &World<S>, batches: [&StorageBatch; 2]) -> Result<()>
where
    S: StorageBackend,
{
