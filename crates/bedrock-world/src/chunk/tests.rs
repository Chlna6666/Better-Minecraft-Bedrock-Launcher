...
fn chunk_key_roundtrips_overworld_and_subchunk() {
    let pos = ChunkPos { x: -3, z: 7, dimension: Dimension::Overworld };
    ...
    assert_eq!(BedrockDbKeyKind::classify(&encoded), BedrockDbKeyKind::Chunk(ChunkRecordTag::SubChunkPrefix));
}

#[test]
fn key_kind_classification_handles_common_non_chunk_keys_without_decoding() {
    assert_eq!(BedrockDbKeyKind::classify(b"~local_player"), BedrockDbKeyKind::LocalPlayer);
    assert_eq!(BedrockDbKeyKind::classify(b"player_123"), BedrockDbKeyKind::RemotePlayer);
    assert_eq!(BedrockDbKeyKind::classify(b"actorprefix12345678"), BedrockDbKeyKind::ActorPrefix);
    assert_eq!(BedrockDbKeyKind::classify(b"unclassified"), BedrockDbKeyKind::Other);
}

#[test]
fn chunk_key_roundtrips_dimension_key() {
    let pos = ChunkPos { x: 1, z: 2, dimension: Dimension::Nether };
...
fn bedrock_db_key_decodes_actor_and_digp_keys() {
    ...
    assert_eq!(BedrockDbKey::decode(&actor_key), BedrockDbKey::ActorPrefix { actor_id: 42 });
    ...
    assert_eq!(
        ...
        BedrockDbKey::ActorDigest { pos: ChunkPos { x: 1, z: -2, dimension: Dimension::Overworld } }
    );
}
...
fn bedrock_db_key_encodes_documented_global_shapes() {
    ...
    assert_eq!(MapRecordId::from_storage_key(b"map_42"), Some(map_id.clone()));
    assert_eq!(BedrockDbKey::Map("42".to_string()).encode().as_deref(), Some(&b"map_42"[..]));

    let pos = ChunkPos { x: 7, z: -8, dimension: Dimension::End };
    let digest = ActorDigestKey::new(pos).storage_key();
    assert_eq!(ActorDigestKey::from_storage_key(&digest), Some(ActorDigestKey::new(pos)));
    assert_eq!(BedrockDbKey::Global(GlobalRecordKind::Scoreboard).encode().as_deref(), Some(&b"scoreboard"[..]));
    assert_eq!(BedrockDbKey::decode(b"TheEnd"), BedrockDbKey::Global(GlobalRecordKind::Dimension(Dimension::End)));
}

#[test]
fn chunk_record_tags_align_with_bedrock_level_reference() {
    let expected = [
        (0x2b, ChunkRecordTag::Data3D), (0x2c, ChunkRecordTag::Version),
        (0x2d, ChunkRecordTag::Data2D), (0x2e, ChunkRecordTag::Data2DLegacy),
        (0x2f, ChunkRecordTag::SubChunkPrefix), (0x30, ChunkRecordTag::LegacyTerrain),
        (0x31, ChunkRecordTag::BlockEntity), (0x32, ChunkRecordTag::Entity),
        (0x33, ChunkRecordTag::PendingTicks), (0x34, ChunkRecordTag::BlockExtraData),
        (0x35, ChunkRecordTag::BiomeState), (0x36, ChunkRecordTag::FinalizedState),
        (0x37, ChunkRecordTag::ConversionData), (0x38, ChunkRecordTag::BorderBlocks),
        (0x39, ChunkRecordTag::HardcodedSpawners), (0x3a, ChunkRecordTag::RandomTicks),
        (0x3b, ChunkRecordTag::Checksums), (0x3c, ChunkRecordTag::GenerationSeed),
        (0x3d, ChunkRecordTag::GeneratedPreCavesAndCliffsBlending),
        (0x3e, ChunkRecordTag::BlendingBiomeHeight), (0x3f, ChunkRecordTag::MetaDataHash),
        (0x40, ChunkRecordTag::BlendingData), (0x41, ChunkRecordTag::ActorDigestVersion),
...
fn bedrock_db_key_decodes_specific_ascii_keys_before_plain_keys() {
    assert_eq!(BedrockDbKey::decode(b"map_42"), BedrockDbKey::Map("42".to_string()));
    assert!(matches!(BedrockDbKey::decode(b"VILLAGE_12345678-1234-1234-1234-123456789abc_INFO"), BedrockDbKey::Village(_)));
    assert!(matches!(BedrockDbKey::decode(b"LevelChunkMetaDataDictionary"), BedrockDbKey::Global(GlobalRecordKind::LevelChunkMetaDataDictionary)));
}

#[test]
fn chunk_pos_matches_bedrock_level_height_ranges() {
    let overworld = ChunkPos { x: 0, z: 0, dimension: Dimension::Overworld };
    ...
    assert_eq!(
        BlockPos { x: -1, y: 64, z: -1 }.to_chunk_pos(Dimension::Overworld),
        ChunkPos { x: -1, z: -1, dimension: Dimension::Overworld }
    );
}
...
fn legacy_terrain_exposes_old_leveldb_arrays() {
    ...
    bytes[LEGACY_TERRAIN_BIOME_OFFSET + column_index * 4..LEGACY_TERRAIN_BIOME_OFFSET + column_index * 4 + 4]
    ...
    assert_eq!(terrain.biome_sample_at(1, 3), Some(LegacyBiomeSample { biome_id: 12, red: 0xab, green: 0xcd, blue: 0xef }));
...
fn legacy_subchunk_decodes_block_ids_metadata_and_light() {
    ...
    let SubChunkFormat::LegacySubChunk(legacy) = &subchunk.format else { panic!("expected legacy subchunk"); };
...
fn paletted_subchunk_v1_uses_single_storage_without_count_byte() {
    ...
    let SubChunkFormat::Paletted { version, storages } = subchunk.format else { panic!("expected v1 paletted subchunk"); };
    assert_eq!(version, 1);
...
fn paletted_subchunk_decodes_supported_bits_per_block() {
    for bits_per_block in [0, 1, 2, 3, 4, 5, 6, 8, 16] {
        ...
        let SubChunkFormat::Paletted { storages, .. } = subchunk.format else { panic!("expected paletted subchunk for {bits_per_block} bits"); };
        assert_eq!(storages.len(), 1);
        assert_eq!(storages[0].indices.as_ref().expect("indices").len(), 4096);
        assert_eq!(storages[0].counts.as_ref().expect("counts").iter().sum::<u16>(), 4096);
    }
}
...
fn paletted_subchunk_counts_only_drops_indices_but_keeps_counts() {
    ...
    let subchunk = parse_subchunk_with_mode(0, Bytes::from(bytes), SubChunkDecodeMode::CountsOnly).expect("parse");
    let SubChunkFormat::Paletted { storages, .. } = subchunk.format else { panic!("expected paletted subchunk"); };
    assert!(storages[0].indices.is_none());
    assert_eq!(storages[0].counts.as_ref().expect("counts-only retains counts").iter().sum::<u16>(), 4096);
}

#[test]
fn surface_columns_keep_random_access_without_full_indices() {
    let bytes = build_paletted_subchunk(8, None, 4, 4);
    let full = parse_subchunk_with_mode(0, Bytes::from(bytes.clone()), SubChunkDecodeMode::FullIndices).expect("parse full indices");
    let surface = parse_subchunk_with_mode(0, Bytes::from(bytes), SubChunkDecodeMode::SurfaceColumns).expect("parse surface columns");
    let SubChunkFormat::Paletted { storages: surface_storages, .. } = &surface.format else { panic!("expected surface paletted subchunk"); };
    ...
    for (x, y, z) in [(0, 0, 0), (1, 2, 3), (15, 15, 15), (7, 9, 4)] {
        assert_eq!(full.block_state_at(x, y, z), surface.block_state_at(x, y, z));
    }
}
...
fn paletted_subchunk_v9_accepts_embedded_y_byte() {
    ...
    let SubChunkFormat::Paletted { storages, .. } = subchunk.format else { panic!("expected paletted v9 subchunk"); };
...
fn paletted_subchunk_v9_accepts_positive_embedded_y_that_looks_like_storage_header() {
    ...
    let SubChunkFormat::Paletted { storages, .. } = &subchunk.format else { panic!("expected paletted v9 subchunk"); };
    assert_eq!(storages[0].states.len(), 4);
    assert_eq!(subchunk.block_state_at(1, 2, 3).expect("block state").name, "minecraft:block_2");
}
...
fn block_state_lookup_uses_xz_plane_storage_order() {
    ...
    assert_eq!(state.name, format!("minecraft:block_{}", block_storage_index(1, 2, 3) % 8));
}

#[test]
fn visible_block_state_lookup_uses_top_non_air_storage() {
    let subchunk = parse_subchunk(0, Bytes::from(build_two_storage_paletted_subchunk("minecraft:stone", "minecraft:copper_block"))).expect("parse layered subchunk");
    assert_eq!(subchunk.block_state_at(1, 2, 3).expect("storage zero state").name, "minecraft:stone");
    let visible = subchunk.visible_block_states_at(1, 2, 3).map(|state| state.name.as_str()).collect::<Vec<_>>();
    assert_eq!(visible, ["minecraft:copper_block", "minecraft:stone"]);
    assert_eq!(subchunk.visible_block_state_at(1, 2, 3).expect("visible state").name, "minecraft:copper_block");
}

#[test]
fn visible_surface_state_iterator_reports_palette_positions() {
    let mut bytes = vec![8, 3];
    append_test_palette_storage(&mut bytes, &["minecraft:air", "minecraft:water"], |x, y, z| u16::from((x, y, z) == (1, 2, 3)));
    append_test_palette_storage(&mut bytes, &["minecraft:air", "minecraft:short_grass"], |x, y, z| u16::from((x, y, z) == (1, 2, 3)));
    append_test_palette_storage(&mut bytes, &["minecraft:air", "minecraft:stone"], |x, y, z| u16::from((x, y, z) == (1, 2, 3)));
    let subchunk = parse_subchunk(0, Bytes::from(bytes)).expect("parse layered subchunk");
    let SubChunkFormat::Paletted { storages, .. } = &subchunk.format else { panic!("expected paletted subchunk"); };
    assert_eq!(storages.len(), 3);
    let visible_entries = subchunk.visible_block_surface_states_at(1, 2, 3).map(|entry| (entry.storage_index, entry.state.name.as_str())).collect::<Vec<_>>();
    assert_eq!(visible_entries, [(2, "minecraft:stone"), (1, "minecraft:short_grass"), (0, "minecraft:water")]);
}

#[test]
fn paletted_subchunk_v9_decodes_zero_bit_secondary_storage_without_palette_len() {
    let mut bytes = vec![9, 2, 4];
    append_test_palette_storage(&mut bytes, &["minecraft:air", "minecraft:stone"], |x, y, z| u16::from((x, y, z) == (4, 2, 4)));
    ...
    assert_eq!(subchunk.block_state_at(4, 2, 4).expect("storage zero state").name, "minecraft:stone");
    assert_eq!(subchunk.visible_block_state_at(4, 2, 4).expect("visible state").name, "minecraft:gold_block");
}
...
fn chunk_get_block_reads_decoded_paletted_subchunk() {
    ...
    let chunk = Chunk {
        pos,
        version: Some(8),
        records: vec![ChunkRecord { key, value: Bytes::from(build_paletted_subchunk(8, None, 4, 8)) }],
    };
...
fn build_paletted_subchunk(version: u8, embedded_y: Option<i8>, bits_per_block: u8, palette_len: usize) -> Vec<u8> {
    let palette_len = if bits_per_block == 0 { 1 } else { palette_len };
    let mut bytes = vec![version, 1];
    if let Some(y) = embedded_y { bytes.push(y as u8); }
    bytes.push(bits_per_block << 1);
    let values_per_word = 32_usize.checked_div(usize::from(bits_per_block)).unwrap_or(4096);
    ...
    if bits_per_block != 0 {
        bytes.extend_from_slice(&i32::try_from(palette_len).expect("palette length").to_le_bytes());
    }
    for index in 0..palette_len {
        let tag = NbtTag::Compound(IndexMap::from([
            ("name".to_string(), NbtTag::String(format!("minecraft:block_{index}"))),
...
fn build_two_storage_paletted_subchunk(lower_name: &str, upper_name: &str) -> Vec<u8> {
    ...
    append_test_palette_storage(&mut bytes, &["minecraft:air", lower_name], |x, y, z| u16::from((x, y, z) == (1, 2, 3)));
    append_test_palette_storage(&mut bytes, &["minecraft:air", upper_name], |x, y, z| u16::from((x, y, z) == (1, 2, 3)));
    bytes
}

fn append_test_palette_storage(bytes: &mut Vec<u8>, palette: &[&str], value_at: impl Fn(u8, u8, u8) -> u16) {
    ...
    bytes.push(bits_per_block << 1);
    for word in words { bytes.extend_from_slice(&word.to_le_bytes()); }
    bytes.extend_from_slice(&i32::try_from(palette.len()).expect("test palette length").to_le_bytes());
    for name in palette {
        let tag = NbtTag::Compound(IndexMap::from([
            ("name".to_string(), NbtTag::String((*name).to_string())),
            ("states".to_string(), NbtTag::Compound(IndexMap::new())),
