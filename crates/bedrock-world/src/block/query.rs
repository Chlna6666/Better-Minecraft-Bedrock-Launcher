    ...
    S: StorageBackend,
{
    ...
    pub fn for_each_block_state_at<F>(
        &self,
        dimension: Dimension,
    ...
    where
        F: for<'a> FnMut(BlockStateView<'a>) -> Result<BlockStateQueryControl>,
    {
        let positions = positions.into_iter().collect::<Vec<_>>();
        self.visit_block_states_at(dimension, &positions, &mut visitor)
    }
    ...
    /// [`World::for_each_block_state_at`]. Server and renderer hot paths should use
    ...
    pub fn block_states(
        ...
        self.visit_block_states_at(dimension, &positions, &mut |entry| {
            ...
            for (index, &(_, subchunk_y)) in subchunk_order.iter().enumerate() {
                let parsed = values
                    ...
                    .map(|bytes| {
                        ...
                        SubChunk::read(subchunk_y, bytes, SubChunkDecodeMode::PackedIndices)
                    })
                    .transpose()?;
...
mod tests {
    ...
    fn empty_batch_is_empty() {
        ...
        assert!(
            world
                .block_states(Dimension::Overworld, [])
                .unwrap()
                .is_empty()
        );
    }
    ...
    fn legacy_terrain_batch_preserves_input_order() {
        ...
        let results = world
            .block_states(Dimension::Overworld, input)
            .expect("query");
