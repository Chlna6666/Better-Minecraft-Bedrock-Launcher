...
impl NativeBlockCache {
    ...
    pub(crate) fn invalidate_paths(&self, paths: &[PathBuf]) {
        ...
        if let Ok(mut inner) = self.inner.lock() {
            let removed_bytes = inner
                .entries
                .iter()
                .filter(|(key, _)| paths.iter().any(|path| path == &key.path))
        ...
        if let Ok(mut indexes) = self.indexes.lock() {
            let removed_bytes = indexes
                .entries
                .iter()
                .filter(|(key, _)| paths.iter().any(|path| path == &key.path))
                .map(|(_, entries)| native_index_entries_size(entries))
                .sum::<usize>();
            indexes
                .entries
                .retain(|key, _| !paths.iter().any(|path| path == &key.path));
            indexes.bytes = indexes.bytes.saturating_sub(removed_bytes);
            indexes
                .order
                .retain(|key| !paths.iter().any(|path| path == &key.path));
        }
        if let Ok(mut files) = self.files.lock() {
            files.retain(|path, _| !paths.iter().any(|obsolete| obsolete == path));
        }
    }
    ...
    fn open_table_file(&self, path: &Path) -> Result<Arc<File>> {
        ...
        let mut files = self
            .files
            .lock()
            .map_err(|_| LevelDbError::lock_poisoned("caching native table file"))?;
        Ok(Arc::clone(
            files
