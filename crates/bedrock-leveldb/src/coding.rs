...
/// Computes CRC32C using the runtime-selected implementation from the `crc32c`
/// crate. On x86/x86_64 this selects SSE4.2 when available and falls back to the
/// portable implementation on older CPUs.
#[inline]
pub(crate) fn crc32c(bytes: &[u8]) -> u32 {
    ::crc32c::crc32c(bytes)
}

/// Computes one LevelDB masked CRC without concatenating the logical chunks.
/// `crc32c_append` keeps the checksum incremental while preserving runtime SIMD
/// dispatch.
