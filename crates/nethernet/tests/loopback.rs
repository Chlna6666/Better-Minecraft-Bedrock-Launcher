...
use bytes::Bytes;
use nethernet::{
    LanSignaling, MAX_SEGMENT_PAYLOAD, NegotiationConfig, NethernetListener, NethernetSession,
    NethernetStream, ServerData,
};
...
async fn connected_pair() -> (NethernetStream, Arc<NethernetSession>, NethernetListener) {
    connected_pair_with_config(NegotiationConfig::default()).await
}

async fn connected_pair_with_config(
    config: NegotiationConfig,
) -> (NethernetStream, Arc<NethernetSession>, NethernetListener) {
    ...
    let mut listener = NethernetListener::bind_with_config(
        server_signaling,
...
async fn minecraft_cpp_trickle_candidates_establish_peer_connection() {
    let (client, server, _listener) =
        connected_pair_with_config(NegotiationConfig::cpp_trickle()).await;
    client.close().await.expect("关闭客户端");
    server.close().await.expect("关闭服务端");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn json_trickle_candidates_establish_peer_connection() {
    let (client, server, _listener) =
        connected_pair_with_config(NegotiationConfig::json_trickle()).await;
    client.close().await.expect("关闭客户端");
    server.close().await.expect("关闭服务端");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn non_trickle_answer_establishes_peer_connection() {
    let (client, server, _listener) =
        connected_pair_with_config(NegotiationConfig::non_trickle()).await;
    client.close().await.expect("关闭客户端");
    server.close().await.expect("关闭服务端");
}
