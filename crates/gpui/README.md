...
This independent fork is pre-1.0 and is maintained as the `gpui` GUI core for
BMCBL's native desktop application framework, `egpui`. It is not an official
Zed distribution. The current renderer direction is nova-gfx:
...
For an application in this repository, use the workspace path dependency:
