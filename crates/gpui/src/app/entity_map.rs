...
#[cfg(any(test, feature = "leak-detection"))]
use leak_detection::{HandleId, LeakDetector};
pub use weak_entity::{AnyWeakEntity, WeakEntity};
...
pub(crate) struct EntityMap {
    ...
    entity_type_names: SecondaryMap<EntityId, &'static str>,
...
impl EntityMap {
    pub fn new() -> Self {
        Self {
            ...
            entity_type_names: SecondaryMap::new(),
        ...
        T: 'static,
    {
        ...
        self.entity_type_names
            .insert(handle.entity_id, type_name::<T>());
    ...
    pub fn type_name_for_id(&self, entity_id: EntityId) -> Option<&'static str> {
        self.entity_type_names.get(entity_id).copied()
    }
    ...
    pub fn take_dropped(&mut self) -> Vec<(EntityId, Box<dyn Any>)> {
        ...
        dropped_entity_ids
            ...
            .filter_map(|entity_id| {
                ...
                self.entity_type_names.remove(entity_id);
...
impl AnyEntity {
    ...
    pub fn downgrade(&self) -> AnyWeakEntity {
        AnyWeakEntity::new(self.entity_id, self.entity_type, self.entity_map.clone())
    }
...
impl<T: 'static> Entity<T> {
    ...
    pub fn downgrade(&self) -> WeakEntity<T> {
        WeakEntity::new(self.any_entity.downgrade(), self.entity_type)
    }
...
mod test {
    ...
    #[test]
    fn erased_lease_round_trip_preserves_typed_entity_and_access_tracking() {
        let mut entity_map = EntityMap::new();
        let slot = entity_map.reserve::<TestEntity>();
    ...
    #[test]
    #[should_panic(expected = "Leases must be ended with EntityMap::end_lease")]
    fn dropping_unended_erased_lease_keeps_lease_invariant() {
        let mut entity_map = EntityMap::new();
        let slot = entity_map.reserve::<TestEntity>();
        let handle = entity_map.insert(slot, TestEntity { i: 1 });
