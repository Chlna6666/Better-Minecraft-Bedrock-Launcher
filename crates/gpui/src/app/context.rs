use crate::{
    ...
    Subscription, Task, WeakEntity, WeakFocusHandle, Window, WindowHandle, WindowVisibility,
};
use anyhow::Result;
...
impl<'a, T: 'static> Context<'a, T> {
    ...
    where
        T: 'static,
        S: Stream + 'static,
        F: FnMut(&mut T, S::Item, &mut Context<T>) + 'static,
    {
...
impl<T> Context<'_, T> {
        ...
        Evt: 'static,
    {
        let event = self
            ...
            .event_arena
            .alloc(|| event)
            .map(|event| event as &mut dyn Any);
