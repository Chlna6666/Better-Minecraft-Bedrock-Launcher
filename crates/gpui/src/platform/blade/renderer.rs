...
static BLADE_PRESENT_COUNT: AtomicU64 = AtomicU64::new(0);
...
#[derive(blade_macros::ShaderData)]
pub(crate) struct ShaderEmojiLayerData {
    pub(crate) layer: EmojiLayerParams,
    pub(crate) t_sprite: gpu::TextureView,
    pub(crate) s_sprite: gpu::Sampler,
}
...
pub(crate) struct EmojiLayerParams {
    pub(crate) atlas_origin: [f32; 2],
    pub(crate) texture_size: [f32; 2],
    pub(crate) layer_origin: [f32; 2],
    pub(crate) layer_size: [f32; 2],
    pub(crate) color: [f32; 4],
}

impl EmojiLayerParams {
    pub(crate) fn new(
        ...
        Self {
            atlas_origin: [tile_bounds.origin.x.0 as f32, tile_bounds.origin.y.0 as f32],
            texture_size: [texture_size.width.0 as f32, texture_size.height.0 as f32],
            layer_origin: [
                layer.bounds.origin.x.0 as f32,
                layer.bounds.origin.y.0 as f32,
            ],
            layer_size: [
                layer.bounds.size.width.0 as f32,
                layer.bounds.size.height.0 as f32,
            ],
...
struct BladePipelines {
    ...
    emoji_layer: gpu::RenderPipeline,
}
...
impl BladePipelines {
    fn new(
        ...
        let shader = gpu
            ...
            .context("Compiling Blade WGSL shader")?;
        ...
        shader.check_struct_size::<EmojiLayerParams>();
        ...
        Ok(Self {
            ...
            emoji_layer: gpu.create_render_pipeline(gpu::RenderPipelineDesc {
                ...
                depth_stencil: None,
                fragment: Some(shader.at("fs_emoji_layer")),
                color_targets: &[gpu::ColorTargetState {
    ...
    fn destroy(&mut self, gpu: &gpu::Context) {
        ...
        gpu.destroy_render_pipeline(&mut self.emoji_layer);
    }
}
...
impl BladeRenderer {
    ...
    >(
        ...
        .context("Creating Blade render pipelines")?;
    ...
    pub fn reconfigure_drawable_size(&mut self, size: Size<DevicePixels>) {
    ...
    pub fn update_transparency(&mut self, is_transparent: bool) {
        if is_transparent != self.surface_config.transparent {
            ...
            match next_pipelines {
                Ok(next_pipelines) => {
                    self.surface_config = next_config;
                    self.pipelines.destroy(&self.gpu);
                    self.pipelines = next_pipelines;
                }
                Err(error) => {
                    log::error!(
                        "Failed to update Blade transparency; keeping previous pipelines: {error:?}"
                    );
                    self.gpu
                        .reconfigure_surface(&mut self.surface, previous_config);
                }
            }
        }
    }
    ...
    fn ensure_path_intermediate_textures(
        ...
        if self.path_intermediate_texture.is_none() {
            ...
            self.path_intermediate_texture_view = Some(path_intermediate_texture_view);
    ...
    fn destroy_path_intermediate_textures(&mut self) {
        if let Some(texture) = self.path_intermediate_texture.take() {
            self.gpu.destroy_texture(texture);
        }
        if let Some(view) = self.path_intermediate_texture_view.take() {
    ...
    fn draw_paths_to_intermediate(
        ...
        let Some((path_texture, path_texture_view, path_msaa_texture, path_msaa_texture_view)) =
            self.ensure_path_intermediate_textures()
        else {
            return false;
        };

        self.command_encoder.init_texture(path_texture);
        if let Some(msaa_texture) = path_msaa_texture {
            self.command_encoder.init_texture(msaa_texture);
        }

        let target = if let Some(msaa_view) = path_msaa_texture_view {
            gpu::RenderTarget {
                view: msaa_view,
                init_op: gpu::InitOp::Clear(gpu::TextureColor::TransparentBlack),
                finish_op: gpu::FinishOp::ResolveTo(path_texture_view),
            }
        } else {
            gpu::RenderTarget {
                view: path_texture_view,
                init_op: gpu::InitOp::Clear(gpu::TextureColor::TransparentBlack),
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        let scene_operation_count = scene.len();
        let scene_primitive_count = scene.shadows.len()
            + scene.quads.len()
            + scene.paths.len()
        ...
        if scene_primitive_count == 0 {
            ...
            if self.clear_only_frames == 3 || self.clear_only_frames % 60 == 0 {
                log::warn!(
                    "Blade presented an empty scene: consecutive_clear_only_frames={} operations={} primitives={} backend=vulkan",
                    self.clear_only_frames,
                    scene_operation_count,
                    scene_primitive_count
                );
            }
        } else if self.clear_only_frames > 0 {
            log::debug!(
                "Blade scene recovered after empty presents: consecutive_clear_only_frames={} operations={} primitives={}",
                self.clear_only_frames,
                scene_operation_count,
                scene_primitive_count
            );
            self.clear_only_frames = 0;
        }
        self.command_encoder.start();
        self.atlas.before_frame(
            &mut self.command_encoder,
            &self.pipelines.emoji_layer,
            self.atlas_sampler,
        );
        ...
        for batch in scene.batches() {
            match batch {
                ...
                PrimitiveBatch::Paths(paths) => {
                    ...
                    drop(pass);
                    let did_draw_paths = self.draw_paths_to_intermediate(
                        paths,
                        self.surface_config.size.width as f32,
                        self.surface_config.size.height as f32,
                    );
                ...
                PrimitiveBatch::BackdropBlurs(blurs) => {
                    for blur in blurs {
                        log::trace!(
                            "Blade backdrop blur queued radius={:?} downsample={}",
                            blur.radius,
                            blur.downsample
                        );
                    }
        ...
        let present_count = BLADE_PRESENT_COUNT.fetch_add(1, Ordering::Relaxed) + 1;
        ...
        record_draw_time(draw_elapsed);
        if draw_elapsed >= std::time::Duration::from_millis(16) {
            log::debug!(
                "Blade draw slow: elapsed_ms={:.3} present_count={} operations={} primitives={}",
