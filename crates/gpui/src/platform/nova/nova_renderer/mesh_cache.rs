...
pub(super) fn custom_mesh_3d_index_byte_offset(entry: NovaMeshCacheEntry) -> u32 {
    entry.index_offset & INDEX_OFFSET_MASK
}

pub(super) fn custom_mesh_3d_index_format(entry: NovaMeshCacheEntry) -> IndexFormat {
...
impl NovaRenderer {
    pub(super) fn ensure_custom_mesh_3d_cache_for_current_backend(&mut self) -> Result<()> {
        ...
        let current_meshes = self.frame_upload.custom_mesh_3d_meshes.clone();
        let current_ids = current_meshes
            .iter()
            .map(|mesh| mesh.id)
            .collect::<FxHashSet<_>>();
        ...
        for mesh in &current_meshes {
            ...
            self.replace_custom_mesh_3d_cache_entry(mesh, &current_ids)?;
        }
    ...
    fn replace_custom_mesh_3d_cache_entry(
        ...
        if reservation.is_none() {
            ...
            for current in self.frame_upload.custom_mesh_3d_meshes.clone() {
