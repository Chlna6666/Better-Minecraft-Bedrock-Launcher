...
pub(super) const PACKED_CUSTOM_MESH_3D_ANIMATION_BYTES: usize = 32;
...
// Static draw parameters and the dynamic animation sidecar intentionally share one per-frame
// allocation. Bindings 20 and 22 expose disjoint storage-buffer views, so animation refreshes can
// update only the compact sidecar without allocating another GPU resource or touching parameters.
pub(super) const CUSTOM_MESH_3D_PARAMETERS_REGION_BYTES: usize =
    MAX_CUSTOM_MESH_3D_DRAWS * PACKED_CUSTOM_MESH_3D_PARAMETERS_BYTES;
pub(super) const CUSTOM_MESH_3D_ANIMATION_REGION_BYTES: usize =
    MAX_CUSTOM_MESH_3D_DRAWS * PACKED_CUSTOM_MESH_3D_ANIMATION_BYTES;
pub(super) const CUSTOM_MESH_3D_FRAME_BUFFER_BYTES: usize =
    CUSTOM_MESH_3D_PARAMETERS_REGION_BYTES + CUSTOM_MESH_3D_ANIMATION_REGION_BYTES;
