...
fn ensure_option<T>(option: &mut Option<T>, value: T) -> &mut T {
    match option {
        Some(current) => current,
        None => option.insert(value),
    }
}
...
use crate::{
    ...
    PlatformInput, PlatformKeyboardLayout, Point, RendererOptions, SCROLL_LINES, ScrollDelta,
    ScrollWheelEvent, Size, TouchPhase, WindowParams, point, px, size,
};
use crate::{
    SharedString,
    platform::linux::{
        LinuxClient, insert_foreground_task_idle, is_within_click_distance, open_uri_internal,
...
pub struct Globals {
    ...
    pub csd_compositor: Option<Arc<CompositorState>>,
    pub csd_subcompositor: Option<Arc<SubcompositorState>>,
...
pub(crate) struct WaylandClientState {
    ...
    client_frame_focused_window: Option<WaylandWindowStatePtr>,
    client_frame_focused_surface: Option<ObjectId>,
...
impl WaylandClientStatePtr {
    pub fn client(&self) -> Rc<RefCell<WaylandClientState>> {
        self.0
            .upgrade()
            .expect("The pointer should always be valid when dispatching in wayland")
    }

    pub fn serial(&self, kind: SerialKind) -> u32 {
...
impl WaylandClient {
    pub(crate) fn new(renderer_options: RendererOptions) -> Self {
        ...
        handle
            .insert_source(main_receiver, {
                ...
                let foreground_task_queue = foreground_task_queue.clone();
        ...
        let mut state = Rc::new(RefCell::new(WaylandClientState {
            ...
            client_frame_focused_window: None,
            client_frame_focused_surface: None,
...
impl LinuxClient for WaylandClient {
    ...
    fn open_window(
        ...
        let text_system = state.common.text_system.clone();
    ...
    fn write_to_primary(&self, item: crate::ClipboardItem) {
        ...
        if state.mouse_focused_window.is_some() || state.keyboard_focused_window.is_some() {
            ...
            let serial = state.serial_tracker.get(SerialKind::Input);
...
impl Dispatch<WlCallback, ObjectId> for WaylandClientStatePtr {
    fn event(
        ...
        let mut state = client.borrow_mut();
        let Some(window) = window_for_surface(&mut state, surface_id) else {
            return;
        };
        drop(state);
...
impl Dispatch<wl_surface::WlSurface, ()> for WaylandClientStatePtr {
    fn event(
        ...
        let Some(window) = window_for_surface(&mut state, &surface.id()) else {
            return;
        };
        #[allow(clippy::mutable_key_type)]
...
impl Dispatch<wl_compositor::WlCompositor, GlobalData> for WaylandClientStatePtr {
    fn event(
        _: &mut Self,
...
impl Dispatch<wl_subcompositor::WlSubcompositor, GlobalData> for WaylandClientStatePtr {
    fn event(
        _: &mut Self,
...
impl Dispatch<wl_subsurface::WlSubsurface, SubsurfaceData> for WaylandClientStatePtr {
    fn event(
        _: &mut Self,
...
impl Dispatch<wl_surface::WlSurface, SurfaceData> for WaylandClientStatePtr {
    fn event(
        this: &mut Self,
        surface: &wl_surface::WlSurface,
        event: <wl_surface::WlSurface as Proxy>::Event,
    ...
    fn event(
        ...
        if let xdg_activation_token_v1::Event::Done { token } = event {
            ...
            match state.pending_activation.take() {
                ...
                Some(PendingActivation::Window(window)) => {
                    let Some(window) = window_for_surface(&mut state, &window) else {
                        return;
                    };
    ...
    fn event(
        ...
        match event {
            ...
            wl_keyboard::Event::Keymap {
                ...
                state.compose_state = xkb_compose_state(&xkb_context);
                drop(state);

                this.handle_keyboard_layout_change();
            }
            wl_keyboard::Event::Enter { surface, .. } => {
                state.keyboard_focused_window = window_for_surface(&mut state, &surface.id());
            ...
            wl_keyboard::Event::Leave { surface, .. } => {
                let keyboard_focused_window = window_for_surface(&mut state, &surface.id());
            ...
            wl_keyboard::Event::Modifiers {
                ...
                if let Some(focused_window) = focused_window {
                    focused_window.dispatch_input(input);
                }
            ...
            wl_keyboard::Event::Key {
                ...
                state.serial_tracker.update(SerialKind::Input, serial);
                        ...
                        state
                            ...
                            .insert_source(Timer::from_duration(state.repeat.delay), {
                                ...
                                move |_event, _metadata, this| {
                                    ...
                                    drop(state);
                                    focused_window.dispatch_input(input.clone());
    ...
    fn event(
        ...
        match event {
            wl_pointer::Event::Enter {
                ...
                if let Some(window) = window_for_surface(&mut state, &surface.id()) {
                    ...
                    let focused = state.windows.values().find_map(|window| {
                        window
                            .handle_client_frame_pointer_motion(
                                &surface_id,
                                surface_x,
                                surface_y,
                                0,
                            )
                            .map(|style| (window.clone(), style))
                    });
                    if let Some((window, style)) = focused {
                        state.client_frame_focused_window = Some(window.clone());
                        state.client_frame_focused_surface = Some(surface_id);
                        state.mouse_location = None;
                        set_client_frame_cursor(&mut state, wl_pointer, serial, style, &window);
                        drop(state);
                        window.set_hovered(true);
                    }
                }
            ...
            wl_pointer::Event::Leave { .. } => {
                if let Some(focused_window) = state.client_frame_focused_window.take() {
                    state.client_frame_focused_surface = None;
                    state.mouse_location = None;
                    state.button_pressed = None;
                    drop(state);
                    focused_window.handle_client_frame_pointer_leave();
                    focused_window.set_hovered(false);
                } else if let Some(focused_window) = state.mouse_focused_window.clone() {
            ...
            wl_pointer::Event::Motion {
                ...
                if let (Some(window), Some(surface_id)) = (
                    state.client_frame_focused_window.clone(),
                    state.client_frame_focused_surface.clone(),
                ) {
                    drop(state);
                    let cursor = window.handle_client_frame_pointer_motion(
                        &surface_id,
                        surface_x,
                        surface_y,
                        time,
            ...
            wl_pointer::Event::Button {
                ...
                if let Some(window) = state.client_frame_focused_window.clone() {
                    let pressed = button_state == wl_pointer::ButtonState::Pressed;
                    drop(state);
                    window.handle_client_frame_pointer_button(time, button, pressed);
                    return;
                }
                if state.mouse_focused_window.is_none() {
                    return;
            ...
            wl_pointer::Event::Axis {
                ...
                let scroll_delta =
                    ensure_option(&mut state.continuous_scroll_delta, point(px(0.0), px(0.0)));
                let modifier = 3.0;
                match axis {
            ...
            wl_pointer::Event::AxisDiscrete {
                ...
                let scroll_delta = ensure_option(&mut state.discrete_scroll_delta, point(0.0, 0.0));
                match axis {
                    wl_pointer::Axis::VerticalScroll => {
        ...
        match event {
            ...
            wl_data_device::Event::Enter {
                ...
                if let Some(data_offer) = data_offer {
                    let Some(drag_window) = window_for_surface(&mut state, &surface.id()) else {
                        return;
                    };
                    ...
                    state
                        ...
                        .spawn(async move {
                            ...
                            drop(state);
                            drag_window.dispatch_input(input);
                        })
                        .detach();
                }
            }
