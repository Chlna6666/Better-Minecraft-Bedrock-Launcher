...
impl PathBuilder {
    ...
    /// Adds a polygon.
    pub fn add_polygon(&mut self, points: &[Point<Pixels>], is_closed: bool) {
    ...
    pub fn build_path(buffers: VertexBuffers<lyon::math::Point, u16>) -> Path<Pixels> {
        ...
        for i in 0..buffers.indices.len() / 3 {
            ...
            let i2 = buffers.indices[i * 3 + 2] as usize;

            let v0 = buffers.vertices[i0];
            let v1 = buffers.vertices[i1];
            let v2 = buffers.vertices[i2];
