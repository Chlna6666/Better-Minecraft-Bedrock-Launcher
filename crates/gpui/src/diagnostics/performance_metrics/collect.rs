...
pub fn performance_metrics_snapshot() -> PerformanceMetricsSnapshot {
    ...
    let image_processing_micros = shared_metrics()
        ...
        .load(Ordering::Relaxed);
    let image_processing_total_micros = shared_metrics()
        .image_processing_total_micros
        .load(Ordering::Relaxed);
    let image_processing_max_micros = shared_metrics()
        .image_processing_max_micros
        .load(Ordering::Relaxed);
    let (
        image_asset_resident_bytes,
        image_asset_retained_count,
        image_asset_largest_resident_bytes,
    ) = shared_metrics()
        ...
        .map(|retained| {
            retained.values().fold(
                ...
                |(total, count, largest), record| {
                    (
                        total.saturating_add(record.resident_bytes),
                        count.saturating_add(1),
                        largest.max(record.resident_bytes),
                    )
                },
            )
        })
        .unwrap_or_default();
    let recent_image_processings = shared_metrics()
    ...
    PerformanceMetricsSnapshot {
        ...
        last_image_processing_compressed_bytes: shared_metrics()
            ...
            .load(Ordering::Relaxed) as usize,
        last_image_processing_output_bytes: shared_metrics()
            .image_processing_output_bytes
            .load(Ordering::Relaxed) as usize,
        last_image_processing_frames: shared_metrics()
            .image_processing_frames
            .load(Ordering::Relaxed) as usize,
        last_image_processing_time: (image_processing_micros > 0)
            .then(|| Duration::from_micros(image_processing_micros)),
        image_processing_count: shared_metrics()
            .image_processing_count
            .load(Ordering::Relaxed) as usize,
        image_processing_compressed_bytes: shared_metrics()
            .image_processing_total_compressed_bytes
            .load(Ordering::Relaxed) as usize,
        image_processing_output_bytes: shared_metrics()
            .image_processing_total_decoded_bytes
            .load(Ordering::Relaxed) as usize,
        image_processing_frames: shared_metrics()
            .image_processing_total_frames
            .load(Ordering::Relaxed) as usize,
        image_processing_total_time: (image_processing_total_micros > 0)
            .then(|| Duration::from_micros(image_processing_total_micros)),
        image_processing_max_time: (image_processing_max_micros > 0)
            .then(|| Duration::from_micros(image_processing_max_micros)),
        image_processing_slow_count: shared_metrics()
            .image_processing_slow_count
            .load(Ordering::Relaxed) as usize,
        animation: animation_metrics_snapshot(),
        image_asset_resident_bytes,
        image_asset_retained_count,
        image_asset_largest_resident_bytes,
        image_asset_cache_entries: image_cache_items.saturating_add(image_asset_retained_count),
        image_asset_compressed_bytes: 0,
