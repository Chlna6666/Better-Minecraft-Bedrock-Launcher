...
#[gpui::test]
fn client_window_control_area_overrides_parent_drag_area(cx: &mut TestAppContext) {
    let window = cx.add_empty_window();
    window.update(|window, cx| {
        let test_window = window.platform_window.as_test().unwrap().clone();
        let drag_hitbox = Hitbox {
        ...
        window.next_hitbox_id = window.next_hitbox_id.next();
        window.rendered_frame.hitboxes.push(drag_hitbox.clone());
        window.rendered_frame.hitboxes.push(client_hitbox.clone());
        window
            .rendered_frame
            .window_control_hitboxes
            .push((WindowControlArea::Drag, drag_hitbox));
        window
            .rendered_frame
            .window_control_hitboxes
            .push((WindowControlArea::Client, client_hitbox));
        ...
        assert!(cx.propagate_event);

        window.dispatch_event(
            PlatformInput::MouseMove(MouseMoveEvent {
                position: point(px(16.), px(10.)),
                pressed_button: Some(MouseButton::Left),
                modifiers: Modifiers::default(),
            }),
            cx,
...
fn gpu_scene_animation_updates_values_without_notifying_view(cx: &mut TestAppContext) {
    ...
    let (test_window, animation_id, baseline_notify_invalidations) =
        window.update(|window, _cx| {
            let test_window = window.platform_window.as_test().unwrap().clone();
            let baseline_notify_invalidations = window.test_dirty_frame_notify_invalidations();
            let element_id = test_global_element_id("scene-animation-rotation");
            let animation_id = window.start_scene_animation(
                ...
                dirty_bounds,
                [0.0; 4],
                [std::f32::consts::TAU, 0.0, 0.0, 0.0],
            );
