    ...
    fn dispatch(&mut self, payload: Bytes, wall_ms: u64, events: &mut Vec<SessionEvent>) {
        match payload.first().copied() {
            Some(ID_CONNECTED_PING) => {
                let Ok(ping) = ConnectedPing::decode(payload) else {
                    return;
                };
                let pong = ConnectedPong {
                    ping_time_ms: ping.time_ms,
                    pong_time_ms: wall_ms,
                };
...
mod tests {
    ...
    fn round_trip_large_split_message() {
        ...
        for _ in 0..2000 {
            ...
            for d in o2 {
                a.ingest(d, now, 0, &mut Vec::new(), &mut Vec::new())
                    .unwrap();
            }
        }
        assert_eq!(got.len(), 1);
    ...
    fn retransmission_recovers_lost_datagram() {
        ...
        for _ in 0..300 {
            ...
            for d in b_out.into_iter().chain(o2) {
                a.ingest(d, now2, 0, &mut Vec::new(), &mut Vec::new())
                    .unwrap();
            }
            if !got.is_empty() {
                break;
    ...
    fn duplicate_datagram_not_delivered_twice() {
        ...
        let mut events = Vec::new();
        b.ingest(dgram.clone(), now, 0, &mut Vec::new(), &mut events)
            .unwrap();
        b.ingest(dgram, now, 0, &mut Vec::new(), &mut events)
            .unwrap();
    ...
    fn ping_answered_with_pong() {
        ...
        let mut a_out = Vec::new();
        for d in o {
            a.ingest(d, later, 999, &mut a_out, &mut Vec::new())
                .unwrap();
        }
