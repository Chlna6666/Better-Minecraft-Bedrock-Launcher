...
use crate::{
    ...
    tunnel::{
        IpScheme, Tunnel, TunnelListener, TunnelScheme, ring::RingTunnelListener,
    ...
    Ok(match l.try_into()? {
        TunnelScheme::Ip(scheme) => match scheme {
            ...
            IpScheme::Quic => {
                ...
                crate::tunnel::quic::QuicTunnelListener::new(l.clone(), global_ctx.clone()).boxed()
            }
            #[cfg(feature = "websocket")]
            IpScheme::Ws | IpScheme::Wss => {
                let mut l = crate::tunnel::websocket::WsTunnelListener::new(l.clone());
                l.set_socket_mark(socket_mark);
                l.boxed()
            }
        },
        #[cfg(unix)]
        TunnelScheme::Unix => crate::tunnel::unix::UnixSocketTunnelListener::new(l.clone()).boxed(),
