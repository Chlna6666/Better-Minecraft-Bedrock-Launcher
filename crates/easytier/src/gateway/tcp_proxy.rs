...
use pnet_packet::tcp::{MutableTcpPacket, TcpPacket, ipv4_checksum};
use quanta::Instant;
...
impl<C: NatDstConnector> TcpProxy<C> {
    ...
    async fn try_handle_peer_packet(&self, packet: &mut ZCPacket) -> Option<()> {
        ...
        let is_tcp_syn = tcp_packet.get_flags() & pnet_packet::tcp::TcpFlags::SYN != 0;
        let is_tcp_ack = tcp_packet.get_flags() & pnet_packet::tcp::TcpFlags::ACK != 0;
