#![cfg_attr(
    ...
    expect(
        unsafe_code,
        reason = "the Windows atlas example owns the native window and raw handle boundary"
    )
)]
...
#[cfg(not(windows))]
fn main() {
    eprintln!("nova-atlas-smoke currently provides a native Win32 Vulkan baseline only");
}
...
";
    ...
    pub fn run() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let vertex_shader = compile_wgsl_to_spirv(ATLAS_WGSL, ShaderStage::Vertex, "vs_main")?;
        let fragment_shader = compile_wgsl_to_spirv(ATLAS_WGSL, ShaderStage::Fragment, "fs_main")?;
    ...
    impl NativeWindow {
        fn new(width: u32, height: u32) -> Result<Self, Box<dyn std::error::Error>> {
            ...
            let class_name = w!("NovaGfxAtlasSmokeWindow");
