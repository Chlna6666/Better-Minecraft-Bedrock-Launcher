#![cfg_attr(
    ...
    expect(
        unsafe_code,
        reason = "the Windows triangle example owns the native window and raw handle boundary"
    )
)]
...
#[cfg(not(windows))]
fn main() {
    eprintln!("nova-triangle currently provides a native Win32 baseline only");
}
...
mod windows_example {
    ...
    use gfx_core::{
        ...
        RenderPipelineDesc, RenderPipelineId, ShaderModuleDesc, ShaderStage, SurfaceConfig,
        SurfaceDesc, SurfaceId, SwapchainId,
    };
...
";
    ...
    pub fn run() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let vertex_shader = compile_wgsl_to_spirv(TRIANGLE_WGSL, ShaderStage::Vertex, "vs_main")?;
        let fragment_shader =
            compile_wgsl_to_spirv(TRIANGLE_WGSL, ShaderStage::Fragment, "fs_main")?;
    ...
    impl NativeWindow {
        fn new(width: u32, height: u32) -> Result<Self, Box<dyn std::error::Error>> {
            ...
            let class_name = w!("NovaGfxTriangleWindow");
