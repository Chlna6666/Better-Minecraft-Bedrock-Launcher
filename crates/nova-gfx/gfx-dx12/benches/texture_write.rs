...
fn main() {
    eprintln!("NOVA_GFX_SKIP=DX12 benchmark requires Windows");
}
...
#[cfg(target_os = "windows")]
fn run(device: &mut Dx12Device) {
    let atlas_size = Extent2d::new(128, 128).expect("atlas extent should be valid");
    let texture = device
        .create_texture(&TextureDesc {
            label: Some("DX12 texture-write benchmark".to_string()),
            size: atlas_size,
    ...
    let mut group = criterion.benchmark_group("native_texture_write/dx12");
...
#[cfg(target_os = "windows")]
fn report_gpu_time(device: &mut Dx12Device, writes: &[TextureWrite<'_>], count: usize) {
    ...
    if let Some(median) = samples.get(samples.len() / 2) {
        eprintln!(
            "NOVA_GFX_GPU_MEDIAN=dx12 batch={count} samples={} nanoseconds={}",
            samples.len(),
            median.as_nanos()
        );
    } else {
        eprintln!("NOVA_GFX_GPU_TIMESTAMP_UNAVAILABLE=dx12 batch={count}");
    }
}
