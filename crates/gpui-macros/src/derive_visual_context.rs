    ...
    let r#gen = quote! {
        ...
        #where_clause
        {
            ...
            fn update_window_entity<T: 'static, R>(
                ...
                gpui::AppContext::update_entity(
                    self.#app_variable,
                    entity,
                    |entity, cx| update(entity, self.#window_variable, cx),
                )
            }

            fn new_window_entity<T: 'static>(
                &mut self,
                build_entity: impl FnOnce(
                    &mut gpui::Window,
                    &mut gpui::Context<'_, T>,
                ) -> T,
            ) -> Self::Result<gpui::Entity<T>> {
                gpui::AppContext::new(
                    self.#app_variable,
                    |cx| build_entity(self.#window_variable, cx),
                )
            }

            fn replace_root_view<V>(
                &mut self,
                build_view: impl FnOnce(
                    &mut gpui::Window,
                    &mut gpui::Context<V>,
                ) -> V,
            ...
            {
                let focus_handle =
                    gpui::Focusable::focus_handle(entity, self.#app_variable);
