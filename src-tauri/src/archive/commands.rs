...
use crate::core::minecraft::key_patcher::patch_path;
...
pub async fn import_appx(
    ...
    // 1. 创建任务并立刻返回 id 给前端
    let task_id = create_task("extracting", None);
    ...
    tokio::spawn(async move {
        ...
        if let Err(e) = fs::create_dir_all(&extract_to) {
            let msg = format!("创建解压目标目录失败：{}，目录：{}", e, extract_to.display());
            error!("{}", msg);
            finish_task(&task_id_for_task, "error", Some(msg));
            return;
        }
        ...
        match extract_zip(archive, extract_to.to_str().unwrap(), true, task_id_for_task.clone()).await {
                ...
                match get_manifest_identity(extract_to.to_str().unwrap()).await {
                    Ok((name, _version)) => {
                        ...
                        if matches!(name.as_str(), "Microsoft.MinecraftUWP" | "Microsoft.MinecraftWindowsBeta") {
                            ...
                            info!("非 Minecraft UWP/WindowsBeta 包（{}），跳过 Manifest 修改", name);
                        }
                    }
                    Err(e) => {
            ...
            Ok(CoreResult::Cancelled) => {
                ...
                if !is_cancelled(&task_id_for_task) {
                    finish_task(&task_id_for_task, "cancelled", Some("user cancelled".into()));
                }
            }
            ...
            Ok(CoreResult::Error(e)) => {
                ...
                finish_task(&task_id_for_task, "error", Some(format!("extract error: {}", e)));
            }

            Err(core_err) => {
                error!("导入解压失败（外层错误）：{}", core_err);
                let _ = fs::remove_dir_all(&extract_to);
                finish_task(&task_id_for_task, "error", Some(format!("extract failed: {}", core_err)));
            }
        }
    });
...
pub async fn extract_zip_appx(
    ...
    tokio::spawn(async move {
        ...
        match extract_zip(archive, extract_to.to_str().unwrap(), force_replace, task_id_clone.clone()).await {
            Ok(CoreResult::Success(())) => {
                ...
                if let Ok(cfg) = read_config() {
                    if !cfg.game.keep_appx_after_install {
                        if let Err(e) = fs::remove_file(&destination_clone) {
                            error!("解压完成后删除源文件失败：{}，路径：{}", e, destination_clone);
                        } else {
                                    ...
                                    if cfg.game.modify_appx_manifest {
                                        match crate::core::minecraft::appx::utils::patch_manifest(&extract_to) {
                                            ...
                                            Err(e) => {
                                                ...
                                                finish_task(&task_id_clone, "error", Some(format!("patch manifest failed: {}", e)));
                                                return;
                                            }
                                        }
                                    }
                                ...
                                if needs_patch {
                                    ...
                                    match tokio::task::spawn_blocking(move || patch_path(&extract_clone)).await {
                                        ...
                                        Ok(Err(patch_err)) => {
                                            ...
                                            // 将具体错误映射为用户可读信息
                                            finish_task(&task_id_clone, "error", Some(format!("patch error: {:?}", patch_err)));
                                            return;
                                        }
                                        Err(join_err) => {
                                            error!("执行补丁任务失败（join error）：{}", join_err);
                                            finish_task(&task_id_clone, "error", Some(format!("patch join error: {}", join_err)));
                                            return;
                                        }
                                    }
                                }
            ...
            Ok(CoreResult::Cancelled) => {
                ...
                // 清理解压目录、源文件等
                if let Err(e) = fs::remove_dir_all(&extract_to) {
                    error!("取消时删除解压目录失败：{}，目录：{}", e, extract_to.display());
                }
                if let Err(e) = fs::remove_file(&destination_clone) {
            ...
            Ok(CoreResult::Error(e)) => {
                error!("解压内部错误：{}", e);
                let _ = fs::remove_dir_all(&extract_to);
                finish_task(&task_id_clone, "error", Some(format!("extract error: {}", e)));
            }

            Err(core_err) => {
                error!("解压失败（外层错误）：{}", core_err);
                let _ = fs::remove_dir_all(&extract_to);
                finish_task(&task_id_clone, "error", Some(format!("extract failed: {}", core_err)));
            }
        }
    });
