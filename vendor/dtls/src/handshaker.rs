...
impl DTLSConn {
    ...
    async fn send(&mut self) -> Result<HandshakeState> {
        ...
        if let Some(pkts) = self.flights.clone() {
            trace!(
                "[handshake:{}] sending DTLS flight {} (records: {})",
                srv_cli_str(self.state.is_client),
                self.current_flight,
                pkts.len()
            );
            ...
            tokio::select! {
                 done_senders = self.handshake_rx.recv() =>{
                    if done_senders.is_none() {
                        warn!("[handshake:{}] DTLS flight {} receiver closed", srv_cli_str(self.state.is_client), self.current_flight);
                        return Err(Error::ErrAlertFatalOrClose);
                    } else if let Some((rendezvous_tx, done_tx)) = done_senders {
                        trace!("[handshake:{}] received DTLS records while waiting for flight {}", srv_cli_str(self.state.is_client), self.current_flight);
                        ...
                        match result {
                            ...
                            Ok(next_flight) => {
                                trace!("[handshake:{}] accepted DTLS flight {} -> {}", srv_cli_str(self.state.is_client), self.current_flight, next_flight);
                ...
                _ = retransmit_timer.as_mut() =>{
                    trace!("[handshake:{}] DTLS flight {} retransmit timer elapsed", srv_cli_str(self.state.is_client), self.current_flight);
