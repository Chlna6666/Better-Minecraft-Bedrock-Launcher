...
pub(crate) fn validate_config(is_client: bool, config: &Config) -> Result<()> {
    ...
    parse_cipher_suites(
        ...
        config.psk.is_none(),
        config.psk.is_some(),
        local_certificate_type(config),
    )?;

    Ok(())
}

pub(crate) fn local_certificate_type(config: &Config) -> Option<ClientCertificateType> {
    config
        .certificates
        .first()
        .map(|certificate| match &certificate.private_key.kind {
            CryptoPrivateKeyKind::Ed25519(_) | CryptoPrivateKeyKind::Ecdsa256(_) => {
                ClientCertificateType::EcdsaSign
            }
            CryptoPrivateKeyKind::Rsa256(_) => ClientCertificateType::RsaSign,
        })
}
