...
pub(crate) fn parse_cipher_suites(
    ...
    certificate_type: Option<ClientCertificateType>,
    ...
    // Do not advertise an authentication suite that the local certificate cannot satisfy.
    let filtered_cipher_suites: Vec<Box<dyn CipherSuite + Send + Sync>> = cipher_suites
        .into_iter()
        .filter(|cipher_suite| {
            !((exclude_psk && cipher_suite.is_psk()) || (exclude_non_psk && !cipher_suite.is_psk()))
                && certificate_type.is_none_or(|certificate_type| {
                    cipher_suite.is_psk() || cipher_suite.certificate_type() == certificate_type
                })
        })
        .collect();
...
#[cfg(test)]
mod tests {
    use super::{default_cipher_suites, parse_cipher_suites, CipherSuiteId, ClientCertificateType};

    #[test]
    fn defaults_keep_supported_certificate_types() {
    ...
    #[test]
    fn local_ecdsa_certificate_excludes_rsa_suites() {
        let suites = parse_cipher_suites(&[], true, false, Some(ClientCertificateType::EcdsaSign))
            .unwrap()
            .into_iter()
            .map(|suite| suite.id())
            .collect::<Vec<_>>();
    ...
    #[test]
    fn client_without_certificate_keeps_rsa_suites() {
        let suites = parse_cipher_suites(&[], true, false, None)
            .unwrap()
            .into_iter()
            .map(|suite| suite.id())
            .collect::<Vec<_>>();

        assert!(suites.contains(&CipherSuiteId::Tls_Ecdhe_Rsa_With_Aes_128_Gcm_Sha256));
        assert!(suites.contains(&CipherSuiteId::Tls_Ecdhe_Rsa_With_Aes_256_Cbc_Sha));
    }
}
