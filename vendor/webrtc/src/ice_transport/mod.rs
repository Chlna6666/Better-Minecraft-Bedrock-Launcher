...
use ice::candidate::{Candidate, CandidatePairState};
...
/// Privacy-safe ICE connectivity diagnostics without candidate addresses or ports.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct RTCIceDiagnostics {
    pub candidate_pair_states: Vec<CandidatePairState>,
...
impl RTCIceTransport {
    ...
    /// Returns connectivity-check counts and pair states without candidate addresses or ports.
    pub async fn get_diagnostics(&self) -> RTCIceDiagnostics {
        let Some(agent) = self.gatherer.get_agent().await else {
            return RTCIceDiagnostics::default();
        };
        let candidate_pair_states = agent
            .get_candidate_pairs_stats()
            .await
            .into_iter()
            .map(|pair| pair.state)
            .collect();
        let checks = agent.get_connectivity_check_stats();
        RTCIceDiagnostics {
            candidate_pair_states,
            requests_received: checks.requests_received,
            requests_sent: checks.requests_sent,
            responses_received: checks.responses_received,
            responses_sent: checks.responses_sent,
        }
    }
