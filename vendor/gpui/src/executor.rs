...
use crate::{record_background_executor_task_finished, record_background_executor_task_scheduled};
...
/// Priority for work scheduled on the background executor.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Hash)]
pub enum Priority {
    /// Realtime work that runs on a dedicated thread instead of the shared pool.
    RealtimeAudio,
    /// Background work that should yield to normal and latency-sensitive work.
    Low,
    /// The default priority for background work.
    #[default]
    Medium,
    /// Latency-sensitive background work that should run before normal work.
    High,
}
...
struct BackgroundQueueGuard;

impl BackgroundQueueGuard {
    fn new() -> Self {
        record_background_executor_task_scheduled();
        Self
    }
}

impl Drop for BackgroundQueueGuard {
    fn drop(&mut self) {
        record_background_executor_task_finished();
    }
}
...
impl BackgroundExecutor {
    ...
    where
        R: Send + 'static,
    {
        self.spawn_with_priority(Priority::Medium, future)
    }
        ...
        R: Send + 'static,
    {
        if priority == Priority::RealtimeAudio {
            self.spawn_realtime(future)
        } else {
            self.spawn_internal::<R>(Box::pin(future), priority, None)
        }
    }

    /// Enqueues the given future on a dedicated realtime thread.
    #[track_caller]
    pub fn spawn_realtime<R>(&self, future: impl Future<Output = R> + Send + 'static) -> Task<R>
    where
        R: Send + 'static,
    {
    ...
    /// Runs a CPU-bound or blocking operation on the background executor.
    ///
    /// The closure must not access foreground-only GPUI state such as [`App`],
    /// [`crate::Window`], or [`crate::Entity`]. Return the result and update UI state
    /// from a foreground task after awaiting it.
    ...
    where
        R: Send + 'static,
    {
        self.spawn_with_priority(priority, async move { operation() })
    }
    ...
    where
        R: Send + 'static,
    {
        self.spawn_internal::<R>(Box::pin(future), Priority::Medium, Some(label))
    }
    ...
    fn spawn_internal<R: Send + 'static>(
        ...
        let dispatcher = self.dispatcher.clone();
        let guard = BackgroundQueueGuard::new();
        ...
        let (runnable, task) = async_task::spawn(future, move |runnable| {
            dispatcher.dispatch_with_priority(runnable, priority, label);
        });
        runnable.schedule();
        Task(TaskState::Spawned(task))
    }
        ...
        R: 'static,
    {
        ...
        fn inner<R: 'static>(
            ...
            let (runnable, task) = spawn_local_with_source_location(future, move |runnable| {
                dispatcher.dispatch_on_main_thread_with_priority(runnable, priority);
            });
            runnable.schedule();
            Task(TaskState::Spawned(task))
        }
        inner::<R>(dispatcher, priority, Box::pin(future))
    }
}
...
mod tests {
    use super::*;
    use crate::{TestDispatcher, performance_metrics_snapshot};
    use futures::channel::oneshot;
    use rand::{SeedableRng, rngs::StdRng};

    #[test]
    fn spawn_blocking_runs_on_background_executor() {
        let dispatcher = Arc::new(TestDispatcher::new(StdRng::seed_from_u64(0)));
        let executor = BackgroundExecutor::new(dispatcher);
        let task = executor.spawn_blocking(Priority::High, || 42);

        assert_eq!(executor.block_test(task), 42);
    }

    #[test]
    fn background_executor_reports_pending_queue_depth() {
        let dispatcher = Arc::new(TestDispatcher::new(StdRng::seed_from_u64(0)));
        let executor = BackgroundExecutor::new(dispatcher);
        let before = performance_metrics_snapshot().background_executor_queue_depth;
        ...
        assert_eq!(
            performance_metrics_snapshot().background_executor_queue_depth,
            before + 1
        );
        tx.send(()).ok();
        assert_eq!(executor.block_test(task), 42);
        ...
        assert!(performance_metrics_snapshot().background_executor_queue_depth_max > before);
    }
}
