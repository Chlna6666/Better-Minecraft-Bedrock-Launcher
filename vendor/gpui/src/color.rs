...
const PATTERN_SLASH_COMPONENT_SCALE: f32 = 255.0;
const PATTERN_SLASH_COMPONENT_MAX: f32 = u16::MAX as f32 / PATTERN_SLASH_COMPONENT_SCALE;

fn encode_pattern_slash_component(component: f32) -> u32 {
    let component = if component.is_finite() {
        component.clamp(0.0, PATTERN_SLASH_COMPONENT_MAX)
    } else {
        0.0
    };

    (component * PATTERN_SLASH_COMPONENT_SCALE).round() as u32
}

/// Creates a hash pattern background
pub fn pattern_slash(color: Hsla, width: f32, interval: f32) -> Background {
    let width_scaled = encode_pattern_slash_component(width);
    let interval_scaled = encode_pattern_slash_component(interval);
    let height = ((width_scaled * u16::MAX as u32) + interval_scaled) as f32;
...
mod tests {
    ...
    #[test]
    fn test_pattern_slash_encodes_dimensions() {
        let color = Hsla::from(rgba(0xff0099ff));
        let background = pattern_slash(color, 1.0, 2.0);

        assert_eq!(background.tag, BackgroundTag::PatternSlash);
        assert_eq!(background.solid, color);
        assert_eq!(
            background.gradient_angle_or_pattern_height,
            ((255 * u16::MAX as u32) + 510) as f32
        );
    }

    #[test]
    fn test_pattern_slash_clamps_invalid_dimensions() {
        let color = Hsla::from(rgba(0xff0099ff));
        let negative_background = pattern_slash(color, -1.0, f32::NAN);
        assert_eq!(negative_background.gradient_angle_or_pattern_height, 0.0);

        let max_component = u16::MAX as u32;
        let large_background = pattern_slash(color, 1000.0, 1000.0);
        assert_eq!(
            large_background.gradient_angle_or_pattern_height,
            ((max_component * u16::MAX as u32) + max_component) as f32
        );
    }
}
