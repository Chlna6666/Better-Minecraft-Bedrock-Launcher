...
impl DirectWriteComponent {
    pub fn new(directx_devices: Option<&DirectXDevices>) -> Result<Self> {
...
impl DirectWriteTextSystem {
    ...
    pub(crate) fn set_directx_devices(&self, directx_devices: &DirectXDevices) -> Result<()> {
        self.0.write().set_directx_devices(directx_devices)
    }
}
...
impl DirectWriteState {
    fn set_directx_devices(&mut self, directx_devices: &DirectXDevices) -> Result<()> {
        self.components.gpu_state = Some(GPUState::new(directx_devices)?);
        Ok(())
    }
    ...
    fn rasterize_glyph(
        ...
        if params.is_emoji {
            ...
            let bytes = self.rasterize_monochrome(params, glyph_bounds)?;
            Ok(GlyphRasterization::Bitmap {
                size: glyph_bounds.size,
                bytes,
            })
    ...
    fn rasterize_color(
        ...
        loop {
            ...
            if image_format == DWRITE_GLYPH_IMAGE_FORMATS_COLR {
                ...
                if color_size.width > 0 && color_size.height > 0 {
                    ...
                    color_layers.push(ColorGlyphLayer {
                        bounds: bounds.map(|value| DevicePixels(value)),
                        color: run_color,
                        alpha: alpha_data.clone(),
                    });
                    if let Some(gpu_state) = self.components.gpu_state.as_ref() {
        ...
        if self.components.gpu_state.is_none() {
            if color_layers.is_empty() {
                anyhow::bail!("no COLR glyph layers were produced");
            }
            return Ok(GlyphRasterization::ColorLayers {
                ...
                fallback: GlyphBitmap {
                    size: glyph_bounds.size,
                    bytes: self.rasterize_color_layers_cpu(glyph_bounds.size, &color_layers),
                },
            });
        }
        ...
        let gpu_state = self
            ...
            .gpu_state
            .as_ref()
            .context("DirectWrite D3D11 color glyph rasterizer is not initialized")?;
        let params_buffer = {
    ...
    fn rasterize_color_layers_cpu(
        ...
        let mut rasterized = vec![0u8; width * height * 4];
        for layer in layers {
            let layer_width = layer.bounds.size.width.0.max(0) as usize;
            let layer_height = layer.bounds.size.height.0.max(0) as usize;
            for y in 0..layer_height {
                let dst_y = layer.bounds.origin.y.0 + y as i32;
                if dst_y < 0 || dst_y as usize >= height {
                    continue;
                }
                for x in 0..layer_width {
                    let dst_x = layer.bounds.origin.x.0 + x as i32;
                    if dst_x < 0 || dst_x as usize >= width {
                        continue;
                    }
                    let alpha = layer.alpha[y * layer_width + x] as f32 / 255.0 * layer.color.a;
                    if alpha <= 0.0 {
                        continue;
                    }
                    let dst = ((dst_y as usize * width) + dst_x as usize) * 4;
                    rasterized[dst] = (layer.color.b * 255.0).round().clamp(0.0, 255.0) as u8;
                    rasterized[dst + 1] =
                        (layer.color.g * 255.0).round().clamp(0.0, 255.0) as u8;
                    rasterized[dst + 2] =
                        (layer.color.r * 255.0).round().clamp(0.0, 255.0) as u8;
                    rasterized[dst + 3] = (alpha * 255.0).round().clamp(0.0, 255.0) as u8;
                }
            }
        }
        rasterized
    ...
    fn handle_gpu_lost(&mut self, directx_devices: &DirectXDevices) {
        try_to_recover_from_device_lost(
            ...
            |gpu_state| self.components.gpu_state = Some(gpu_state),
            || {
                log::error!(
