...
use super::window::{ignore_invalid_window_handle_error, is_valid_window_handle};
...
impl WindowsWindowInner {
    ...
    fn handle_hit_test_msg(
        ...
        let position = {
            let scale_factor = self.state.borrow().scale_factor;
            let mut cursor_point = POINT {
                x: lparam.signed_loword().into(),
                y: lparam.signed_hiword().into(),
            };
        ...
        if let Some(area) = self.state.borrow().hit_test_window_control_area(position) {
            return match area {
                Some(WindowControlArea::Drag) => Some(HTCAPTION as _),
                Some(WindowControlArea::Close) => Some(HTCLOSE as _),
                Some(WindowControlArea::Max) => Some(HTMAXBUTTON as _),
                Some(WindowControlArea::Min) => Some(HTMINBUTTON as _),
                None => Some(HTCLIENT as _),
            };
        }
    ...
    fn handle_nc_mouse_down_msg(
        ...
        if button == MouseButton::Left {
            match wparam.0 as u32 {
                HTMINBUTTON => {
                    self.state.borrow_mut().nc_button_pressed = Some(HTMINBUTTON);
                    return Some(0);
                }
                HTMAXBUTTON => {
                    self.state.borrow_mut().nc_button_pressed = Some(HTMAXBUTTON);
                    return Some(0);
                }
                HTCLOSE => {
                    self.state.borrow_mut().nc_button_pressed = Some(HTCLOSE);
                    return Some(0);
                }
                _ => {}
            }
    ...
    fn handle_nc_mouse_up_msg(
        ...
        let last_pressed = if button == MouseButton::Left {
            self.state.borrow_mut().nc_button_pressed.take()
        } else {
            None
        };
        if let Some(last_pressed) = last_pressed {
            let handled = match (wparam.0 as u32, last_pressed) {
                (HTMINBUTTON, HTMINBUTTON) => {
                    ...
                    unsafe {
                        ignore_invalid_window_handle_error(
                            ShowWindowAsync(handle, SW_MINIMIZE).ok(),
                        );
                    };
                    true
                }
                (HTMAXBUTTON, HTMAXBUTTON) => {
                    ...
                    if self.state.borrow().is_maximized() {
                        unsafe {
                            ignore_invalid_window_handle_error(
                                ShowWindowAsync(handle, SW_NORMAL).ok(),
                            );
                        };
                    } else {
                        unsafe {
                            ignore_invalid_window_handle_error(
                                ShowWindowAsync(handle, SW_MAXIMIZE).ok(),
                            );
                        };
                    }
                    true
                ...
                (HTCLOSE, HTCLOSE) => {
                    ...
                    unsafe {
                        ignore_invalid_window_handle_error(PostMessageW(
                            Some(handle),
                            WM_CLOSE,
                            WPARAM::default(),
                            LPARAM::default(),
