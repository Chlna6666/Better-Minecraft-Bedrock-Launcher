...
use crate::{
    performance_metrics::{
        record_atlas_texture_count, record_backdrop_blur_primitive_count, record_draw_time,
    },
...
pub(crate) struct DirectXRenderer {
    ...
    gpu_mesh_3d_cache: HashMap<GpuMesh3dId, DirectXGpuMesh3dBuffer>,
...
struct DirectXResources {
    ...
    backdrop_blur_texture: ID3D11Texture2D,
    backdrop_blur_srv: [Option<ID3D11ShaderResourceView>; 1],
    gpu_mesh_3d_depth_texture: ID3D11Texture2D,
    gpu_mesh_3d_depth_view: [Option<ID3D11DepthStencilView>; 1],
...
struct DirectXRenderPipelines {
    ...
    backdrop_blur_pipeline: PipelineState<BackdropBlurSprite>,
    path_rasterization_pipeline: PipelineState<PathRasterizationSprite>,
    path_sprite_pipeline: PipelineState<PathSprite>,
    underline_pipeline: PipelineState<Underline>,
    mono_sprites: PipelineState<MonochromeSprite>,
    poly_sprites: PipelineState<PolychromeSprite>,
    gpu_mesh_3d: PipelineState<GpuMesh3dVertex>,
    gpu_mesh_3d_depth_write: ID3D11DepthStencilState,
    gpu_mesh_3d_depth_read: ID3D11DepthStencilState,
}

struct DirectXGlobalElements {
    global_params_buffer: [Option<ID3D11Buffer>; 1],
    gpu_mesh_3d_params_buffer: [Option<ID3D11Buffer>; 1],
    sampler: [Option<ID3D11SamplerState>; 1],
}

struct DirectXGpuMesh3dBuffer {
    generation: u64,
    vertex_count: usize,
    buffer: ID3D11Buffer,
...
struct GpuMesh3dParams {
    ...
    center: [f32; 3],
    fit_scale: f32,
    yaw_pitch_zoom_ndc_aspect: [f32; 4],
    vertical_scale_offset_pad: [f32; 4],
    depth_scale_pad: [f32; 4],
}
...
impl DirectXRenderer {
    ...
    pub(crate) fn draw(&mut self, scene: &Scene) -> Result<()> {
        ...
        self.pre_draw()?;
        record_backdrop_blur_primitive_count(scene.backdrop_blurs.len());
        record_atlas_texture_count(self.atlas.texture_count());
        for batch in scene.batches() {
            match batch {
                ...
                PrimitiveBatch::BackdropBlurs(blurs) => {
                    self.capture_backdrop_texture();
                    self.draw_backdrop_blurs(blurs)
                }
                PrimitiveBatch::GpuMeshes3d(meshes) => self.draw_gpu_meshes_3d(meshes),
        ...
        record_draw_time(draw_started.elapsed());
        result
    }
    ...
    fn draw_backdrop_blurs(&mut self, blurs: &[PaintBackdropBlur]) -> Result<()> {
        if blurs.is_empty() {
            return Ok(());
        }
        let sprites = blurs
            .iter()
            .map(|blur| BackdropBlurSprite {
                order: blur.order,
                downsample: blur.downsample as u32,
                bounds: blur.bounds,
                content_mask: blur.content_mask.clone(),
                corner_radii: blur.corner_radii.clone(),
                tint: blur.tint.unwrap_or_else(transparent_black),
                radius: f64::from(blur.radius) as f32,
        ...
        self.pipelines.backdrop_blur_pipeline.draw_with_texture(
            &self.devices.device_context,
            &self.resources.backdrop_blur_srv,
            &self.resources.viewport,
            &self.globals.global_params_buffer,
            &self.globals.sampler,
            sprites.len() as u32,
    ...
    fn capture_backdrop_texture(&mut self) {
        unsafe {
            self.devices.device_context.CopyResource(
                &self.resources.backdrop_blur_texture,
                &*self.resources.render_target,
            );
        }
    }

    fn draw_gpu_meshes_3d(&mut self, meshes: &[PaintGpuMesh3d]) -> Result<()> {
        for mesh in meshes {
            self.draw_gpu_mesh_3d(mesh)?;
        }
        unsafe {
            self.devices
                .device_context
    ...
    fn draw_gpu_mesh_3d(&mut self, painted: &PaintGpuMesh3d) -> Result<()> {
        if painted.mesh.vertices.is_empty() {
            return Ok(());
        }

        let entry = match self.gpu_mesh_3d_cache.entry(painted.mesh.id) {
            Entry::Occupied(entry) => entry.into_mut(),
            Entry::Vacant(entry) => {
                let buffer = create_buffer(
                    &self.devices.device,
                    std::mem::size_of::<GpuMesh3dVertex>(),
                    painted.mesh.vertices.len().max(1),
                )?;
                let view = create_buffer_view(&self.devices.device, &buffer)?;
                entry.insert(DirectXGpuMesh3dBuffer {
                    generation: u64::MAX,
                    vertex_count: 0,
                    buffer,
                    view,
        ...
        if entry.generation != painted.mesh.generation
            || entry.vertex_count != painted.mesh.vertices.len()
        {
            if entry.vertex_count < painted.mesh.vertices.len() {
                entry.buffer = create_buffer(
                    &self.devices.device,
                    std::mem::size_of::<GpuMesh3dVertex>(),
                    painted.mesh.vertices.len().next_power_of_two(),
                )?;
                entry.view = create_buffer_view(&self.devices.device, &entry.buffer)?;
            }
            update_buffer(
                &self.devices.device_context,
                &entry.buffer,
                &painted.mesh.vertices,
            )?;
            entry.generation = painted.mesh.generation;
            entry.vertex_count = painted.mesh.vertices.len();
        }

        let params = GpuMesh3dParams {
            bounds_origin: [painted.bounds.origin.x.0, painted.bounds.origin.y.0],
            bounds_size: [painted.bounds.size.width.0, painted.bounds.size.height.0],
            content_mask_origin: [
                painted.content_mask.bounds.origin.x.0,
                painted.content_mask.bounds.origin.y.0,
            ],
            content_mask_size: [
                painted.content_mask.bounds.size.width.0,
                painted.content_mask.bounds.size.height.0,
            ],
            center: painted.mesh.center,
            fit_scale: painted.mesh.fit_scale,
            yaw_pitch_zoom_ndc_aspect: [
                painted.camera.yaw,
                painted.camera.pitch,
                painted.camera.zoom,
                painted.mesh.ndc_x_aspect,
            ],
            vertical_scale_offset_pad: [
                painted.mesh.vertical_scale,
                painted.mesh.vertical_offset,
                0.0,
                0.0,
            ],
            depth_scale_pad: [painted.mesh.depth_scale, 0.0, 0.0, 0.0],
        };
        update_buffer(
            &self.devices.device_context,
            self.globals.gpu_mesh_3d_params_buffer[0].as_ref().unwrap(),
            &[params],
        )?;
        unsafe {
            self.devices.device_context.ClearDepthStencilView(
                self.resources.gpu_mesh_3d_depth_view[0].as_ref().unwrap(),
                D3D11_CLEAR_DEPTH.0,
                1.0,
                0,
            );
            self.devices.device_context.OMSetRenderTargets(
                Some(&self.resources.render_target_view),
                self.resources.gpu_mesh_3d_depth_view[0].as_ref(),
            );
        }

        let global_params = [
            self.globals.global_params_buffer[0].clone(),
            self.globals.gpu_mesh_3d_params_buffer[0].clone(),
        ];
        self.pipelines.gpu_mesh_3d.draw_mesh_ranges(
            &self.devices.device_context,
            &entry.view,
            &self.resources.viewport,
            &global_params,
            painted.mesh.ranges,
            &self.pipelines.gpu_mesh_3d_depth_write,
            &self.pipelines.gpu_mesh_3d_depth_read,
        )
    }
...
impl DirectXResources {
    pub fn new(
        ...
        let (
            ...
            backdrop_blur_srv,
            gpu_mesh_3d_depth_texture,
            gpu_mesh_3d_depth_view,
            viewport,
    ...
    fn recreate_resources(
        ...
        self.backdrop_blur_texture = backdrop_blur_texture;
        self.backdrop_blur_srv = backdrop_blur_srv;
        self.gpu_mesh_3d_depth_texture = gpu_mesh_3d_depth_texture;
        self.gpu_mesh_3d_depth_view = gpu_mesh_3d_depth_view;
...
impl DirectXRenderPipelines {
    pub fn new(device: &ID3D11Device) -> Result<Self> {
        ...
        let gpu_mesh_3d = PipelineState::new(
            device,
            "gpu_mesh_3d_pipeline",
            ShaderModule::GpuMesh3d,
            1,
            create_blend_state(device)?,
        )?;
        let gpu_mesh_3d_depth_write = create_depth_stencil_state(device, true)?;
        let gpu_mesh_3d_depth_read = create_depth_stencil_state(device, false)?;
        ...
        Ok(Self {
            ...
            poly_sprites,
            gpu_mesh_3d,
            gpu_mesh_3d_depth_write,
            gpu_mesh_3d_depth_read,
        })
    }
}
...
impl DirectXGlobalElements {
    pub fn new(device: &ID3D11Device) -> Result<Self> {
        ...
        let gpu_mesh_3d_params_buffer = unsafe {
            let desc = D3D11_BUFFER_DESC {
                ByteWidth: std::mem::size_of::<GpuMesh3dParams>() as u32,
                Usage: D3D11_USAGE_DYNAMIC,
                BindFlags: D3D11_BIND_CONSTANT_BUFFER.0 as u32,
                CPUAccessFlags: D3D11_CPU_ACCESS_WRITE.0 as u32,
                ..Default::default()
...
impl<T> PipelineState<T> {
    ...
    fn draw_mesh_ranges(
        &self,
        device_context: &ID3D11DeviceContext,
        mesh_view: &[Option<ID3D11ShaderResourceView>],
        viewport: &[D3D11_VIEWPORT],
        global_params: &[Option<ID3D11Buffer>],
        ranges: GpuMesh3dDrawRanges,
        depth_write: &ID3D11DepthStencilState,
        ...
        unsafe {
            if ranges.opaque.count > 0 {
                device_context.OMSetDepthStencilState(depth_write, 0);
                device_context.DrawInstanced(ranges.opaque.count, 1, ranges.opaque.start, 0);
            }
            device_context.OMSetDepthStencilState(depth_read, 0);
            if ranges.glass.count > 0 {
                device_context.DrawInstanced(ranges.glass.count, 1, ranges.glass.start, 0);
            }
            if ranges.water.count > 0 {
                device_context.DrawInstanced(ranges.water.count, 1, ranges.water.start, 0);
            }
...
fn create_resources(
    ...
    [Option<ID3D11ShaderResourceView>; 1],
    ID3D11Texture2D,
    [Option<ID3D11DepthStencilView>; 1],
    [D3D11_VIEWPORT; 1],
)> {
    ...
    let (backdrop_blur_texture, backdrop_blur_srv) =
        create_sampled_render_target_texture(&devices.device, width, height)?;
    let (gpu_mesh_3d_depth_texture, gpu_mesh_3d_depth_view) =
        create_gpu_mesh_3d_depth_texture_and_view(&devices.device, width, height)?;
...
fn create_path_intermediate_texture(
    ...
    create_sampled_render_target_texture(device, width, height)
}

#[inline]
fn create_sampled_render_target_texture(
    device: &ID3D11Device,
    width: u32,
    height: u32,
) -> Result<(ID3D11Texture2D, [Option<ID3D11ShaderResourceView>; 1])> {
...
#[inline]
fn create_gpu_mesh_3d_depth_texture_and_view(
    device: &ID3D11Device,
    width: u32,
    height: u32,
) -> Result<(ID3D11Texture2D, [Option<ID3D11DepthStencilView>; 1])> {
    ...
    unsafe { device.CreateDepthStencilView(&depth_texture, None, Some(&mut depth_view))? };
    Ok((depth_texture, [Some(depth_view.unwrap())]))
}
    ...
    impl<'t> RawShaderBytes<'t> {
        ...
        fn from_bytes(module: ShaderModule, target: ShaderTarget) -> Self {
            let bytes = match module {
                ...
                ShaderModule::GpuMesh3d => match target {
                    ShaderTarget::Vertex => GPU_MESH_3D_VERTEX_BYTES,
                    ShaderTarget::Fragment => GPU_MESH_3D_FRAGMENT_BYTES,
                },
    ...
    impl ShaderModule {
        pub fn as_str(&self) -> &str {
            match self {
                ...
                ShaderModule::GpuMesh3d => "gpu_mesh_3d",
