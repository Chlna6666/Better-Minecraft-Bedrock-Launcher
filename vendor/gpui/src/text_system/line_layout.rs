use crate::{
    BackgroundExecutor, FontId, GlyphId, GpuiMemoryTrimLevel, Pixels, PlatformTextSystem, Point,
    Priority, SharedString, Size, point, px, record_text_layout_cache_metrics,
};
...
pub(crate) struct LineLayoutCache {
    ...
    background_cache: Arc<BackgroundLineLayoutCache>,
...
#[derive(Default)]
pub(crate) struct BackgroundLineLayoutCache {
    lines: RwLock<FxHashMap<Arc<CacheKey>, Arc<LineLayout>>>,
    wrapped_lines: RwLock<FxHashMap<Arc<CacheKey>, Arc<WrappedLineLayout>>>,
}

impl BackgroundLineLayoutCache {
    pub(crate) fn clear(&self) {
        self.lines.write().clear();
        self.wrapped_lines.write().clear();
    }

    pub(crate) fn insert_line_layout(
        &self,
        text: SharedString,
        font_size: Pixels,
        runs: &[FontRun],
        ...
        self.lines.write().insert(key, layout);
    }

    fn get_line_layout(&self, key: &dyn AsCacheKeyRef) -> Option<Arc<LineLayout>> {
        self.lines.read().get(key).cloned()
    }

    fn contains_line_layout(&self, key: &dyn AsCacheKeyRef) -> bool {
        self.lines.read().contains_key(key)
    }

    pub(crate) fn insert_wrapped_line_layout(
        &self,
        text: SharedString,
        font_size: Pixels,
        runs: &[FontRun],
        ...
        self.wrapped_lines.write().insert(key, layout);
    }

    fn get_wrapped_line_layout(&self, key: &dyn AsCacheKeyRef) -> Option<Arc<WrappedLineLayout>> {
        self.wrapped_lines.read().get(key).cloned()
    }

    fn contains_wrapped_line_layout(&self, key: &dyn AsCacheKeyRef) -> bool {
        self.wrapped_lines.read().contains_key(key)
    }
}
...
impl LineLayoutCacheMetrics {
    ...
    fn finish_frame(&self) {
        record_text_layout_cache_metrics(self.hits.get(), self.reuses.get(), self.misses.get());
        self.hits.set(0);
        self.reuses.set(0);
        self.misses.set(0);
    }
...
impl LineLayoutCache {
    ...
    pub fn new(platform_text_system: Arc<dyn PlatformTextSystem>) -> Self {
        Self::with_background_cache(
            platform_text_system,
            Arc::new(BackgroundLineLayoutCache::default()),
            None,
        )
    }

    pub fn with_background_cache(
        platform_text_system: Arc<dyn PlatformTextSystem>,
        background_cache: Arc<BackgroundLineLayoutCache>,
        background_executor: Option<BackgroundExecutor>,
    ) -> Self {
    ...
    pub fn trim_retained_capacity_for_level(&self, level: GpuiMemoryTrimLevel) {
        ...
        previous_frame.lines.shrink_to_fit();
        previous_frame.wrapped_lines.shrink_to_fit();
        previous_frame.used_lines.shrink_to_fit();
        previous_frame.used_wrapped_lines.shrink_to_fit();
        current_frame.lines.shrink_to_fit();
        current_frame.wrapped_lines.shrink_to_fit();
        current_frame.used_lines.shrink_to_fit();
        current_frame.used_wrapped_lines.shrink_to_fit();
    }
        ...
        SharedString: From<Text>,
    {
        ...
        if let Some((key, layout)) = previous_frame_entry {
            ...
            self.frame_metrics.reuse();
            layout
        } else if let Some(layout) = self.background_cache.get_wrapped_line_layout(key) {
            let mut current_frame = RwLockUpgradableReadGuard::upgrade(current_frame);
            let cache_key = key.as_cache_key_ref();
            let key = Arc::new(CacheKey {
                text: SharedString::new(cache_key.text),
                font_size: cache_key.font_size,
                runs: SmallVec::from(cache_key.runs),
                wrap_width: cache_key.wrap_width,
                max_lines: cache_key.max_lines,
                force_width: cache_key.force_width,
            });
            current_frame
                .wrapped_lines
                .insert(key.clone(), layout.clone());
            ...
            current_frame.used_wrapped_lines.push(key.clone());
            drop(current_frame);
            self.prewarm_wrapped_line_layout(key);
        ...
        SharedString: From<Text>,
    {
        ...
        let key = &cache_key as &dyn AsCacheKeyRef;
        ...
        if let Some((key, layout)) = self.previous_frame.lock().lines.remove_entry(key) {
            ...
            self.frame_metrics.reuse();
            layout
        } else if let Some(layout) = self.background_cache.get_line_layout(key) {
            let mut current_frame = RwLockUpgradableReadGuard::upgrade(current_frame);
            let key = Arc::new(CacheKey {
                text: SharedString::new(cache_key.text),
                font_size: cache_key.font_size,
            ...
            current_frame.used_lines.push(key.clone());
            drop(current_frame);
            self.prewarm_line_layout(key);
            layout
        }
    }

    fn prewarm_line_layout(&self, key: Arc<CacheKey>) {
        if key.text.len() < 32 || self.background_cache.contains_line_layout(key.borrow()) {
            return;
        }

        let Some(background_executor) = self.background_executor.clone() else {
            return;
        };

        let platform_text_system = self.platform_text_system.clone();
        let background_cache = self.background_cache.clone();
        background_executor
            .spawn_with_priority(Priority::Low, async move {
                let mut layout =
                    platform_text_system.layout_line(&key.text, key.font_size, &key.runs);
                if let Some(force_width) = key.force_width {
                    let mut glyph_pos = 0;
                    for run in layout.runs.iter_mut() {
                        for glyph in run.glyphs.iter_mut() {
                            if (glyph.position.x - glyph_pos * force_width).abs() > px(1.) {
                ...
                background_cache.insert_line_layout(
                    key.text.clone(),
                    key.font_size,
                    &key.runs,
                    key.force_width,
    ...
    fn prewarm_wrapped_line_layout(&self, key: Arc<CacheKey>) {
        if key.wrap_width.is_none()
            || key.text.len() < 32
            || self
                .background_cache
                .contains_wrapped_line_layout(key.borrow())
        {
            return;
        }
        ...
        background_executor
            .spawn_with_priority(Priority::Low, async move {
                ...
                background_cache.insert_wrapped_line_layout(
                    key.text.clone(),
                    key.font_size,
                    &key.runs,
                    key.wrap_width,
...
mod tests {
    use super::*;
    use crate::{NoopTextSystem, TestDispatcher, performance_metrics_snapshot, px};
    ...
    #[test]
    fn wrapped_line_cache_includes_max_lines() {
        let cache = LineLayoutCache::new(Arc::new(NoopTextSystem::new()));
        let text = "hello world from gpui";
        let runs = [FontRun {
            len: text.len(),
            font_id: FontId(1),
        }];

        let clamped = cache.layout_wrapped_line(text, px(14.), &runs, Some(px(4.)), Some(1));
        let unclamped = cache.layout_wrapped_line(text, px(14.), &runs, Some(px(4.)), Some(3));

        assert!(!Arc::ptr_eq(&clamped, &unclamped));
        assert!(clamped.wrap_boundaries.len() < unclamped.wrap_boundaries.len());
    }

    #[test]
    fn layout_line_reuses_background_warm_up_layout() {
        let background_cache = Arc::new(BackgroundLineLayoutCache::default());
        let runs = [FontRun {
            len: 5,
            font_id: FontId(1),
        }];
        let warmed = Arc::new(LineLayout {
            font_size: px(14.),
            width: px(42.),
            ascent: px(10.),
            descent: px(4.),
        ...
        let cache = LineLayoutCache::with_background_cache(
            Arc::new(NoopTextSystem::new()),
            background_cache,
            None,
        );
        let layout = cache.layout_line("hello", px(14.), &runs, None);
        cache.finish_frame();

        let metrics = performance_metrics_snapshot();
        assert!(Arc::ptr_eq(&layout, &warmed));
        assert_eq!(metrics.text_layout_reuses, 1);
        assert_eq!(metrics.text_layout_misses, 0);
    }

    #[test]
    fn long_layout_miss_is_prewarmed_on_background_executor() {
        ...
        let text = "hello gpui background layout prewarm data";

        let cache = LineLayoutCache::with_background_cache(
            platform_text_system.clone(),
            background_cache.clone(),
            Some(executor),
        );
        cache.layout_line(text, px(14.), &runs, None);
        ...
        let warmed = background_cache
            .get_line_layout(&key)
            .expect("background prewarm should insert line layout");

        let second_cache =
            LineLayoutCache::with_background_cache(platform_text_system, background_cache, None);
        let layout = second_cache.layout_line(text, px(14.), &runs, None);

        assert!(Arc::ptr_eq(&layout, &warmed));
    }

    #[test]
    fn wrapped_layout_miss_is_prewarmed_on_background_executor() {
        let dispatcher = Arc::new(TestDispatcher::new(StdRng::seed_from_u64(0)));
        let executor = BackgroundExecutor::new(dispatcher.clone());
        let background_cache = Arc::new(BackgroundLineLayoutCache::default());
        let platform_text_system = Arc::new(NoopTextSystem::new());
        let text = "hello gpui background wrapped layout prewarm data";
        let runs = [FontRun {
            len: text.len(),
            font_id: FontId(1),
        }];
        ...
        cache.layout_wrapped_line(text, px(14.), &runs, Some(px(4.)), Some(3));
        dispatcher.run_until_parked();
        let key = CacheKeyRef {
            text,
            font_size: px(14.),
            runs: &runs,
            wrap_width: Some(px(4.)),
            max_lines: Some(3),
            force_width: None,
        };
        let warmed = background_cache
            .get_wrapped_line_layout(&key)
            .expect("background prewarm should insert wrapped line layout");

        let second_cache =
            LineLayoutCache::with_background_cache(platform_text_system, background_cache, None);
        let layout = second_cache.layout_wrapped_line(text, px(14.), &runs, Some(px(4.)), Some(3));
