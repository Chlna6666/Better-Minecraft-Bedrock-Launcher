...
impl Arena {
    ...
    pub fn trim_to_max_capacity(&mut self, max_capacity: usize) -> bool {
        self.clear();

        if self.capacity() <= max_capacity || self.chunks.len() <= 1 {
            return false;
        }

        while self.chunks.len() > 1 && self.capacity() > max_capacity {
            self.chunks.pop();
        }
        self.current_chunk_index = 0;
        true
...
mod tests {
    ...
    #[test]
    fn test_arena_trim_to_max_capacity() {
        let mut arena = Arena::new(8);
        arena.alloc(|| 1u64);
        arena.alloc(|| 2u64);
        arena.alloc(|| 3u64);
        ...
        assert!(arena.trim_to_max_capacity(8));
