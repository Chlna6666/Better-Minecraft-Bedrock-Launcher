...
use crate::{
    ...
    IsZero, LayoutId, ListSizingBehavior, Overflow, Pixels, Point, RenderFingerprint, ScrollHandle,
    Size, StyleRefinement, Styled, Window, point, size,
};
use smallvec::SmallVec;
use std::{cell::RefCell, cmp, hash::Hash, ops::Range, rc::Rc};
            ...
            |style, window, cx| match self.sizing_behavior {
                ListSizingBehavior::Infer => {
                    window.with_text_style(style.text_style().cloned(), |window| {
                        ...
                        item_size.hash(&mut hasher);
                        max_items.hash(&mut hasher);
                        window.request_pure_measured_layout_with_fingerprint(
                            style,
                            hasher.value(),
