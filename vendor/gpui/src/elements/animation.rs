...
struct AnimationBoundsState {
    last_bounds: Option<crate::Bounds<crate::Pixels>>,
}
...
impl<E: IntoElement + 'static> Element for AnimationElement<E> {
    ...
    fn prepaint(
        ...
        window.with_element_state(id.unwrap(), |state, window| {
            let mut state = state.unwrap_or_else(|| AnimationBoundsState { last_bounds: None });
            let dirty_bounds = state
                .last_bounds
                .map_or(bounds, |last_bounds| bounds.union(&last_bounds));
            state.last_bounds = Some(bounds);
            window.mark_animation_dirty(dirty_bounds);
            ((), state)
        });
