use crate::{
    ...
    MouseDownEvent, MouseMoveEvent, MouseUpEvent, Pixels, Point, RenderFingerprint, SharedString,
    Size, TextOverflow, TextRun, TextStyle, TooltipId, WhiteSpace, Window, WrappedLine,
...
pub struct TextLayout(
    ...
    Rc<RefCell<Option<TextLayoutFallback>>>,
);

#[derive(Clone)]
struct TextLayoutFallback {
    cache_key: u64,
    text: SharedString,
    runs: Vec<TextRun>,
...
impl TextLayout {
    fn layout(
        ...
        self.1.borrow_mut().replace(TextLayoutFallback {
            cache_key,
            text: text.clone(),
            runs: runs.clone(),
    ...
    fn measure(
        ...
        let text = if let Some(truncate_width) = truncate_width {
            line_wrapper.truncate_line(text, truncate_width, &truncation_suffix, &mut runs)
        } else {
            text
        };
        let len = text.len();
    ...
    fn restore_measurement_from_prepaint(
        &self,
        bounds: Bounds<Pixels>,
        window: &mut Window,
        cx: &mut App,
    ) -> bool {
        let Some(fallback) = self.1.borrow().clone() else {
            return false;
        };

        let wrap_width = if fallback.text_style.white_space == WhiteSpace::Normal {
            Some(bounds.size.width)
        } else {
            None
        };
        let truncate_width = fallback.text_style.text_overflow.as_ref().map(|_| {
            match fallback.text_style.line_clamp {
                Some(max_lines) => bounds.size.width * max_lines,
                None => bounds.size.width,
            }
        });
        let truncation_suffix = fallback
            .text_style
            .text_overflow
            .as_ref()
            .map(|text_overflow| match text_overflow {
                TextOverflow::Truncate(suffix) => suffix.clone(),
            })
            .unwrap_or_else(|| "".into());
    ...
    fn prepaint(&self, bounds: Bounds<Pixels>, text: &str, window: &mut Window, cx: &mut App) {
        let missing_measurement = self.0.borrow().is_none();
        if missing_measurement && !self.restore_measurement_from_prepaint(bounds, window, cx) {
            log::warn!("gpui text layout missing measurement and fallback: text={text}");
            return;
        }
        ...
        if let Some(element_state) = element_state.as_mut() {
            ...
            log::warn!("gpui text layout measurement fallback failed: text={text}");
        }
    }
    ...
    fn paint(&self, text: &str, window: &mut Window, cx: &mut App) {
        ...
        let Some(element_state) = element_state.as_ref() else {
            log::warn!("gpui text paint skipped missing measurement: text={text}");
            return;
        };
        let Some(bounds) = element_state.bounds else {
            log::warn!("gpui text paint skipped missing prepaint: text={text}");
            return;
        };
...
mod tests {
    use super::*;
    use crate::{AvailableSpace, TestAppContext, point, px, size};
    ...
    #[gpui::test]
    fn text_layout_prepaint_restores_missing_measurement(cx: &mut TestAppContext) {
        let layout = TextLayout::default();
        let (_, visual) = cx.add_window_view(|_, _| crate::Empty);

        struct MissingMeasurementText {
            layout: TextLayout,
            text: SharedString,
        }

        impl IntoElement for MissingMeasurementText {
            type Element = Self;
        ...
        impl Element for MissingMeasurementText {
            type RequestLayoutState = ();
            type PrepaintState = ();
            ...
            fn request_layout(
                ...
                (self.layout.layout(self.text.clone(), None, window, cx), ())
            }
        ...
        visual.draw(
            point(px(0.), px(0.)),
            size(
                AvailableSpace::Definite(px(160.)),
                AvailableSpace::MinContent,
            ),
            |_, _| MissingMeasurementText {
                layout: layout.clone(),
                text: SharedString::from("启动"),
            },
        );

        let state = layout.0.borrow();
        let state = state.as_ref().expect("prepaint should restore measurement");
        assert!(state.size.is_some());
    ...
    fn text_layout_remains_measured_across_retained_layout_draws(cx: &mut TestAppContext) {
        ...
        let first_layout = TextLayout::default();
        let mut first = StyledText::new("common.not_installed");
        first.layout = first_layout.clone();
        visual.draw(point(px(0.), px(0.)), available_space, |_, _| first);

        let first_size = first_layout
            .0
            .borrow()
            .as_ref()
        ...
        let second_layout = TextLayout::default();
        let mut second = StyledText::new("common.not_installed");
        second.layout = second_layout.clone();
        visual.draw(point(px(0.), px(0.)), available_space, |_, _| second);

        let second_size = second_layout
            .0
            .borrow()
            .as_ref()
            .and_then(|state| state.size);
