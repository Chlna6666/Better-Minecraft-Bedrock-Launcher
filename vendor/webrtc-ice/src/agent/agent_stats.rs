...
/// Aggregate STUN connectivity-check counters for one ICE agent.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct ConnectivityCheckStats {
...
impl AgentInternal {
    pub(crate) fn get_connectivity_check_stats(&self) -> ConnectivityCheckStats {
        ConnectivityCheckStats {
            requests_received: self.binding_requests_received.load(Ordering::Relaxed),
            requests_sent: self.binding_requests_sent.load(Ordering::Relaxed),
            responses_received: self.binding_responses_received.load(Ordering::Relaxed),
            responses_sent: self.binding_responses_sent.load(Ordering::Relaxed),
        }
    }
