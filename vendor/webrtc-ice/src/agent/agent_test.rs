...
async fn test_connectivity_on_startup() -> Result<()> {
    ...
    let accepting_agent = Arc::clone(&a_agent);
    tokio::spawn(async move {
        let result = accepting_agent.accept(a_cancel_rx, b_ufrag, b_pwd).await;
    ...
    for stats in [
        a_agent.get_connectivity_check_stats(),
        b_agent.get_connectivity_check_stats(),
    ] {
