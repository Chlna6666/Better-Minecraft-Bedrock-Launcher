...
mod tests {
    ...
    #[test]
    fn default_layout_contains_all_standard_window_controls() {
        let mut buttons = Buttons::new(None);
        buttons.arrange(500, 0.0);

        assert_eq!(
            buttons.find_button(483.0, 17.0),
            Location::Button(ButtonKind::Close)
        );
        assert_eq!(
            buttons.find_button(446.0, 17.0),
            Location::Button(ButtonKind::Maximize)
        );
        assert_eq!(
            buttons.find_button(409.0, 17.0),
