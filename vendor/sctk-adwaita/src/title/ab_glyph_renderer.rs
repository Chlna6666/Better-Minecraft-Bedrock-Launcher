...
impl AbGlyphTitleText {
    pub fn new(color: Color) -> Self {
        ...
        Self {
            title: <_>::default(),
            font,
            font_point_size: font_pref_pt_size,
    ...
    pub fn update_title(&mut self, title: impl Into<String>) {
        ...
        if new_title != self.title {
            if !font_supports_title(parse_font(&self.font), &new_title) {
                if let Some(font) = font_matching_title(&new_title, self.font_point_size) {
                    self.font = Some(font);
                    if let Some(size) = parse_font(&self.font).pt_to_px_scale(self.font_point_size)
                    {
                        self.original_px_size = size.x;
                        self.size = PxScale::from(size.x * self.scale as f32);
                    }
                }
            }
...
fn font_matching_title(title: &str, point_size: f32) -> Option<(memmap2::Mmap, FontPreference)> {
    let charset = title
        .chars()
    ...
    let pattern = format!("sans-serif:charset={charset}");
    let paths = Command::new("fc-match")
        .args(["--sort", "-f", "%{file}\n"])
        .arg(pattern)
        .output()
        .ok()
    ...
    for path in paths.lines().filter(|path| !path.is_empty()) {
        ...
        let Some(mmap) = mmap(&file) else {
            continue;
        };
        let Ok(font) = FontRef::try_from_slice(&mmap) else {
            continue;
        };
        if font_supports_title(font, title) {
            return Some((
                mmap,
                FontPreference {
                    name: "fontconfig fallback".into(),
                    style: None,
                    pt_size: point_size,
                },
            ));
...
fn font_supports_title(font: FontRef<'_>, title: &str) -> bool {
    title.chars().all(|character| {
        character.is_control() || character.is_whitespace() || font.glyph_id(character).0 != 0
    })
}
...
mod tests {
    ...
    #[test]
    fn title_font_fallback_covers_chinese_and_ascii() {
        let title = "地图预览 - Bedrock";
        let Some((font, _)) = font_matching_title(title, 11.0) else {
            return;
        };

        let font = FontRef::try_from_slice(&font).expect("fallback must be a supported font file");
        assert!(font_supports_title(font, title));
    }
}
