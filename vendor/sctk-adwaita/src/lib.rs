    ...
    State: Dispatch<WlSurface, SurfaceData> + Dispatch<WlSubsurface, SubsurfaceData> + 'static,
{
    pub fn new(
        ...
        Ok(AdwaitaFrame {
            ...
            theme,
            buttons: Buttons::new(if frame_config.use_system_button_layout {
            ...
            use_system_button_layout: frame_config.use_system_button_layout,
        })
    }

    /// Update the current frame config.
    pub fn set_config(&mut self, config: FrameConfig) {
        if self.use_system_button_layout != config.use_system_button_layout {
            self.use_system_button_layout = config.use_system_button_layout;
            self.buttons = Buttons::new(if self.use_system_button_layout {
                get_button_layout_config()
            } else {
                None
            });
            self.buttons.update_wm_capabilities(self.wm_capabilities);
        }
        self.theme = config.theme;
        self.dirty = true;
    }

    pub fn set_title_mask(&mut self, mask: TitleMask) -> bool {
        let colors = if self.state.contains(WindowState::ACTIVATED) {
            &self.theme.active
        } else {
            &self.theme.inactive
        };
        let Some(title_text) = self.title_text.as_mut() else {
            return false;
        };
        if !title_text.set_mask(mask, colors.font_color) {
            return false;
        }
        self.dirty = true;
        true
    ...
    pub fn clear_title_mask(&mut self) {
        if self.title_text.as_mut().is_some_and(TitleText::clear_mask) {
            self.dirty = true;
        }
    }
...
pub struct FrameConfig {
    ...
    pub use_system_button_layout: bool,
}
...
impl FrameConfig {
    ...
    pub fn new(theme: ColorTheme) -> Self {
        Self {
            theme,
            use_system_button_layout: true,
        }
    }

    /// Choose whether window buttons follow the desktop environment layout.
    ///
    /// Disabling this uses the standard minimize, maximize, and close buttons.
    pub fn use_system_button_layout(mut self, use_system_button_layout: bool) -> Self {
        self.use_system_button_layout = use_system_button_layout;
        self
    }
