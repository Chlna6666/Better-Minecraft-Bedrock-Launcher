...
use crate::downloads::wu_client::protocol::{WuDownloadFile, WuProtocol};
use crate::result::{CoreError, CoreResult};
use crate::tasks::task_manager::{finish_task, is_cancelled, update_progress, wait_until_active};
...
impl WuClient {
    ...
    pub async fn get_download_url(
        ...
        match self
            .get_download_files(update_id, revision, task_id)
            .await?
        {
            CoreResult::Success(files) => files
                .into_iter()
                .map(|file| file.url)
                .next()
                .map(CoreResult::Success)
                .ok_or(CoreError::BadUpdateIdentity),
            CoreResult::Cancelled => Ok(CoreResult::Cancelled),
            CoreResult::Error(error) => Ok(CoreResult::Error(error)),
        }
    }
    ...
    pub async fn get_download_urls(
        ...
        revision: &str,
        task_id: &str,
    ) -> Result<CoreResult<Vec<String>>, CoreError> {
        match self
            .get_download_files(update_id, revision, task_id)
            .await?
        {
            CoreResult::Success(files) => Ok(CoreResult::Success(
                files.into_iter().map(|file| file.url).collect(),
            )),
            CoreResult::Cancelled => Ok(CoreResult::Cancelled),
            CoreResult::Error(error) => Ok(CoreResult::Error(error)),
        }
    ...
    pub async fn get_download_files(
        ...
        for attempt in 1..=3 {
            ...
            if !wait_until_active(task_id).await {
                ...
                Ok(resp) => {
                    ...
                    match resp.error_for_status() {
                        Ok(valid_resp) => {
                            ...
                            let files = match self.protocol.parse_download_response(&xml) {
                                Ok(u) => u,
                                Err(e) => {
                            ...
                            debug!("task={} 解析到的 URL 列表: {:?}", task_id, files);
                            ...
                            if !files.is_empty() {
                                ...
                                return Ok(CoreResult::Success(files));
                            } else {
                                // 没有匹配的 URL
                                if attempt == 3 {
