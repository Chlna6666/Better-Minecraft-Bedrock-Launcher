...
pub(crate) fn read_packs_standard(
    ...
    let pack_roots = game_target_dirs(options, pack_target_dir(kind)?);
    ...
    let results: Vec<McPackInfo> = pack_folders
        ...
        .filter_map(|(folder_path, base_path)| {
            ...
            let icon_path = if icon_abs.exists() {
                Some(icon_abs.to_string_lossy().to_string())
            } else {
                None
            };
...
fn pack_target_dir(kind: &str) -> Result<GameTargetDir> {
    match kind {
        "resource_packs" => Ok(GameTargetDir::ResourcePacks),
        "behavior_packs" => Ok(GameTargetDir::BehaviorPacks),
        _ => bail!("不支持的资源包目录类型: {kind}"),
    }
}
...
fn collect_pack_folders_filtered(
    ...
    if let Some(iso_id) = isolation_id {
        ...
        if is_gdk {
            ...
            let pack_root = version_base.join("games").join("com.mojang").join(kind);
...
pub fn replace_header_with_lang(manifest_value: &mut Value, lang_map: &HashMap<String, String>) {
    if let Value::Object(root) = manifest_value {
        if let Some(Value::Object(header)) = root.get_mut("header") {
            if let Some(Value::String(name_s)) = header.get("name") {
                if let Some(repl) = lang_map.get(name_s) {
                    header.insert("name".to_string(), Value::String(repl.clone()));
                }
            }
            if let Some(Value::String(desc_s)) = header.get("description") {
        ...
        if let Some(Value::Array(modules)) = root.get_mut("modules") {
            for m in modules {
                if let Value::Object(mod_obj) = m {
                    if let Some(Value::String(desc_s)) = mod_obj.get("description") {
                        if let Some(repl) = lang_map.get(desc_s) {
