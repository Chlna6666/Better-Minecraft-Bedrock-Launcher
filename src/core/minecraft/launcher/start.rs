...
pub fn is_process_in_package(pid: u32, target_family_name: &str) -> bool {
    unsafe {
        let Ok(h_proc) = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, false, pid) else {
            return false;
        };
        ...
        if res == WIN32_ERROR(0) {
            let family = String::from_utf16_lossy(&buffer)
                .trim_matches('\0')
                .to_string();
...
pub async fn launch_uwp_command_only(
    app_user_model_id: &str,
    launch_args: Option<&str>,
) -> WindowsResult<Option<u32>> {
    debug!(
        "Executing launch_uwp_command_only: AUMID={}, Args={:?}",
        app_user_model_id, launch_args
    );
    unsafe {
    ...
    if let Some(args) = launch_args {
        ...
        if args.contains("://") {
            ...
            let pfn = app_user_model_id
                .split('!')
                .next()
                .unwrap_or(app_user_model_id);
            let options = LauncherOptions::new()?;
            options.SetTargetApplicationPackageFamilyName(&HSTRING::from(pfn))?;
            let launched = Launcher::LaunchUriWithOptionsAsync(&uri, &options)?.await?;
            return Ok(launched.then_some(0));
        }
    }
    ...
    let args_hstring = if let Some(a) = launch_args {
        HSTRING::from(a)
    } else {
        HSTRING::new()
    };
...
pub async fn wait_for_uwp_pid(target_exe: &str, package_family_name: &str) -> Option<u32> {
    ...
    while start.elapsed() < Duration::from_secs(15) {
        ...
        for pid in &current_pids {
            if !pids_before.contains(pid) && is_process_in_package(*pid, package_family_name) {
                return Some(*pid);
            }
        }
...
pub async fn launch_uwp(edition: &str, launch_args: Option<&str>) -> io::Result<Option<u32>> {
    ...
    let target_exe_name = if edition.contains("Education") {
