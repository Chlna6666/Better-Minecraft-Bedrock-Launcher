...
use gpui::image::ImageFormat as DecodedImageFormat;
use rayon::prelude::*;
...
#[derive(Debug, Serialize, Clone)]
pub struct PreviewIconData {
    pub format: PreviewImageFormat,
    pub bytes: Vec<u8>,
}

fn image_format_from_mime(mime: &str) -> PreviewImageFormat {
    match mime {
        "image/jpeg" => PreviewImageFormat::Jpeg,
        "image/webp" => PreviewImageFormat::Webp,
        "image/gif" => PreviewImageFormat::Gif,
        "image/bmp" => PreviewImageFormat::Bmp,
        _ => PreviewImageFormat::Png,
    }
}

fn image_format_from_bytes(bytes: &[u8]) -> PreviewImageFormat {
...
fn normalize_preview_icon(bytes: Vec<u8>) -> Result<(PreviewIconData, u32, u32)> {
    let decoded =
        gpui::image::load_from_memory(&bytes).context("failed to decode preview icon bytes")?;
    let width = decoded.width();
    let height = decoded.height();

    let mut encoded = Cursor::new(Vec::new());
    decoded
        .write_to(&mut encoded, DecodedImageFormat::Png)
        .context("failed to normalize preview icon into png")?;
...
pub struct PackagePreview {
    ...
    pub icon: Option<PreviewIconData>,
    ...
    pub world_pack_references: Option<Vec<WorldPackReference>>,
    pub valid: bool,                    // [新增] 规范校验
    pub invalid_reason: Option<String>, // [新增]
}

#[derive(Debug, Serialize, Clone)]
pub struct WorldPackReference {
...
fn primary_world_root(scan: &ArchiveScanResult) -> Option<&str> {
    ...
    if scan.level_roots.len() == 1 {
        return scan.level_roots.first().map(String::as_str);
    }

    None
}

fn is_world_archive(scan: &ArchiveScanResult) -> bool {
    primary_world_root(scan).is_some() && scan.packs.is_empty() && !scan.has_nested_archive
}

fn resolve_world_archive_primary(scan: &ArchiveScanResult) -> Option<String> {
    let world_root = primary_world_root(scan)?;
    if scan.has_nested_archive {
        return None;
    }

    let normalized_world_root = normalize_root_key(world_root);
    for pack in &scan.packs {
        let root = normalize_root_key(&pack.root);
        let is_allowed = if normalized_world_root.is_empty() {
            root.starts_with("resource_packs/")
                || root.starts_with("behavior_packs/")
                || root.starts_with("skin_packs/")
        } else {
            root.starts_with(&format!("{normalized_world_root}resource_packs/"))
                || root.starts_with(&format!("{normalized_world_root}behavior_packs/"))
                || root.starts_with(&format!("{normalized_world_root}skin_packs/"))
        };
        if !is_allowed {
            return None;
        }
...
fn version_to_string(version: &ManifestVersion) -> String {
    match version {
        ManifestVersion::Numbers(numbers) => numbers
            .iter()
            .map(u32::to_string)
            .collect::<Vec<_>>()
            .join("."),
        ManifestVersion::String(version) => version.clone(),
    }
}

fn world_pack_type_display_name(file_name: &str) -> &'static str {
    if file_name.eq_ignore_ascii_case("world_behavior_packs.json") {
        ImportTargetType::BehaviorPack.to_display_name()
    } else {
        ImportTargetType::ResourcePack.to_display_name()
    }
}

fn parse_world_pack_reference_version(value: &serde_json::Value) -> Option<String> {
    match value {
...
fn parse_world_pack_references_from_text(
    content: &str,
    file_name: &str,
) -> Vec<WorldPackReference> {
    let cleaned = strip_json_comments(content.trim_start_matches('\u{feff}'));
    ...
    items
        ...
        .filter_map(|(index, item)| {
            ...
            let uuid = object
                ...
                .or_else(|| object.get("uuid").and_then(serde_json::Value::as_str))?
                .to_string();

            Some(WorldPackReference {
                pack_type: world_pack_type_display_name(file_name).to_string(),
                order: index + 1,
                uuid,
                version: object
                    .get("version")
                    .and_then(parse_world_pack_reference_version),
                subpack: object
                    .get("subpack")
                    .and_then(serde_json::Value::as_str)
...
fn world_pack_references_from_zip<R: Read + Seek>(
    archive: &mut ZipArchive<R>,
    root_prefix: &str,
) -> Vec<WorldPackReference> {
    let mut references = Vec::new();
    for file_name in ["world_behavior_packs.json", "world_resource_packs.json"] {
        let path = format!("{root_prefix}{file_name}");
        if let Some(content) = read_zip_text_case_insensitive(archive, &path) {
            references.extend(parse_world_pack_references_from_text(&content, file_name));
        }
    }
    references
}
...
pub fn inspect_archive(path: &Path, preferred_lang: Option<&str>) -> Result<PackagePreview> {
    ...
    // 1. 基础分析 (确定类型和默认名称)
    let (target_type, default_name, _uuid, scan) = analyze_archive(&mut archive, path)?;
    ...
    let mut world_pack_references = Vec::new();
    let mut valid = true;
    ...
    if effective_type == ImportTargetType::World {
        // --- 存档处理 ---
        if let Some(world_root) = resolve_world_archive_primary(&scan)
            .or_else(|| primary_world_root(&scan).map(normalize_root_key))
        {
            debug!(
                "Inspect world preview: archive={:?}, root={}, embedded_packs={}",
                path,
                world_root,
                scan.packs.len()
            );
            if let Ok(world_preview) = get_world_info_from_zip(&mut archive, &world_root) {
                if !world_preview.name.trim().is_empty() {
                    name = world_preview.name;
                }
                if !world_preview.description.trim().is_empty() {
                    description = world_preview.description;
                }
                if world_preview.icon.is_some() {
                    icon = world_preview.icon;
                }
                if let Some(references) = world_preview.world_pack_references {
                    world_pack_references = references;
                }
                debug!(
                    "Inspect world preview resolved: archive={:?}, name={}, icon={}, refs={}",
                    path,
                    name,
                    icon.is_some(),
                    world_pack_references.len()
                );
            }
        }
        ...
        if !scan.has_nested_archive {
            ...
            let filtered_level_roots =
                filter_level_roots_excluding_template_internal(&scan.level_roots, &scan.packs);
            ...
            if sub_packs.is_empty() {
                ...
                let (work_dir, pack_dirs) =
                    match extract_to_cache_with_nested(&mut archive, "inspect") {
                        ...
                        Err(e) => {
                            return Err(e)
                                .context("Failed to extract compound archive for inspection");
                        }
                    };
                ...
                for dir in pack_dirs {
                    if dir.join("manifest.json").is_file() {
                        if let Ok(preview) =
                            get_pack_info_from_dir(&dir, &ImportTargetType::Unknown, preferred_lang)
                        {
                            sub_packs.push(preview);
                        }
        ...
        if !sub_packs.is_empty() {
            let primary = sub_packs
                .iter()
                .find(|p| p.kind == ImportTargetType::WorldTemplate.to_display_name())
        ...
        if effective_type == ImportTargetType::WorldTemplate {
            if let Some(pack) = template_pack.as_ref().or_else(|| {
                scan.packs
                    .iter()
                ...
                if let Some(s) = read_zip_text_by_name(&mut archive, &pack.manifest_path) {
                    ...
                    read_zip_text_case_insensitive(&mut archive, &pack.manifest_path)
                {
                    manifest_content = Some(s);
                }
            }
        ...
        if let Some(content) = manifest_content {
            let clean = strip_json_comments(content.trim_start_matches('\u{feff}'));
            if let Ok(mut manifest) = serde_json::from_str::<PartialManifest>(&clean) {
                ...
                if let Some(header) = manifest.header.as_mut() {
                    ...
                    if let Some(v) = &header.version {
                        version_str = Some(version_to_string(v));
                    }
        ...
        if let Some(buf) = icon_buf {
            ...
            icon = Some(PreviewIconData {
                format: image_format_from_mime(mime),
                bytes: buf,
            });
        }

        if effective_type == ImportTargetType::WorldTemplate {
            let template_root = normalize_root_key(&root_prefix);
            world_pack_references = world_pack_references_from_zip(&mut archive, &template_root);
            let mut seen = std::collections::HashSet::new();
            for pack in &scan.packs {
                ...
                let pack_root = normalize_root_key(&pack.root);
                let is_internal_pack = if template_root.is_empty() {
                    pack_root.starts_with("resource_packs/")
                        || pack_root.starts_with("behavior_packs/")
                        || pack_root.starts_with("skin_packs/")
                } else {
                    pack_root.starts_with(&format!("{template_root}resource_packs/"))
                        || pack_root.starts_with(&format!("{template_root}behavior_packs/"))
                        || pack_root.starts_with(&format!("{template_root}skin_packs/"))
                };
                if !is_internal_pack || !seen.insert(pack.manifest_path.clone()) {
                    continue;
                }
            ...
            if icon.is_none() {
                icon = sub_packs.iter().find_map(|preview| preview.icon.clone());
            }
            if sub_packs.iter().any(|preview| !preview.valid) {
                valid = false;
                invalid_reason = Some("存在子包缺少 UUID".to_string());
            }

            debug!(
                "Inspect world template preview resolved: archive={:?}, root={}, sub_packs={}, refs={}",
                path,
                root_prefix,
                sub_packs.len(),
                world_pack_references.len()
    ...
    Ok(PackagePreview {
        ...
        kind: if effective_type == ImportTargetType::Compound && !sub_packs.is_empty() {
            ...
            primary
                .map(|p| p.kind.clone())
                .unwrap_or_else(|| effective_type.to_display_name().to_string())
        } else {
        ...
        sub_packs: if sub_packs.is_empty() {
            None
        } else {
            Some(sub_packs)
        },
        world_pack_references: if world_pack_references.is_empty() {
            None
        } else {
            Some(world_pack_references)
        },
        valid,
        invalid_reason,
    })
        ...
        if let Some(new_uuid) = &pack_uuid {
            if let Some(existing_uuid) = get_pack_uuid_from_dir(&current_dest) {
                if existing_uuid == *new_uuid {
                    let existing_pack_info =
                        get_pack_info_from_dir(&current_dest, &target_type, None).ok();
                    let target_name = existing_pack_info
                        .as_ref()
                        .map(|p| p.name.clone())
...
fn process_single_archive(
    ...
    if let ImportTargetType::Unknown = target_type {
        return Err(anyhow::anyhow!(
            "无法识别包类型: 缺少 manifest.json 或 level.dat"
        ));
    }
            ...
            if let Some(existing_uuid) = get_pack_uuid_from_dir(&final_dest) {
                if existing_uuid == *new_uuid {
                    if overwrite {
                        ...
                        return Err(anyhow::anyhow!(
                            "Pack with same UUID exists (overwrite denied)"
                        ));
                    }
                }
            }
...
pub fn scan_archive<R: Read + Seek>(archive: &mut ZipArchive<R>) -> Result<ArchiveScanResult> {
    ...
    for i in 0..archive.len() {
        ...
        if let Ok(mut file) = archive.by_index(i) {
            ...
            let clean = strip_json_comments(json.trim_start_matches('\u{feff}'));
    ...
    for h in hits {
        match h {
            ScanHit::LevelDat { root } => {
                level_roots.insert(root);
            }
    ...
    let is_world = level_roots.contains("") && !packs.iter().any(|p| p.root.is_empty());
...
pub fn group_packs(
    ...
    let mut map: std::collections::HashMap<ImportTargetType, Vec<PackEntry>> =
...
pub fn analyze_archive(
    ...
    if ext == "mctemplate" {
            ...
            .or_else(|| scan.packs.first())
        {
            ...
            let uuid = p.manifest.header.as_ref().and_then(|h| h.uuid.clone());
            debug!(
            ...
            return Ok((ImportTargetType::WorldTemplate, name, uuid, scan));
        }
    }
    ...
    if let Some(world_root) = resolve_world_archive_primary(&scan) {
        debug!(
            "Analyze archive: {:?}, result=World (with embedded packs), packs={}, compound={}, nested={}, root={}, elapsed={} ms",
            original_path,
            scan.packs.len(),
            scan.is_compound,
            scan.has_nested_archive,
        ...
        return Ok((ImportTargetType::World, default_name, None, scan));
    }

    if is_world_archive(&scan) {
        debug!(
    ...
    if scan.is_compound || scan.has_nested_archive {
        ...
        return Ok((ImportTargetType::Compound, default_name, None, scan));
    }
    ...
    let pack = scan
        .packs
        .first()
        .ok_or_else(|| anyhow::anyhow!("Invalid archive: no manifest found"))?;
    ...
    let uuid = pack.manifest.header.as_ref().and_then(|h| h.uuid.clone());
    ...
    Ok((pack.pack_type.clone(), name, uuid, scan))
}
    ...
    if let Some(modules) = &manifest.modules {
        for module in modules {
            match module.module_type.as_str() {
                ...
                "data" | "behaviorpack" | "script" | "javascript" | "client_data" | "interface" => {
    ...
    if let Some(ref k) = key {
        if let Some(work_dir) = cache_take_compound(k) {
            if work_dir.exists() {
                debug!(
                    "Compound cache hit: {:?} -> {:?}",
                    original_file_path, work_dir
                );
                let mut _inner_archives = Vec::new();
                let mut pack_dirs = Vec::new();
                collect_inner_archives_and_dirs(
                    &work_dir,
                    &mut _inner_archives,
                    &mut pack_dirs,
                    2,
...
fn extract_subdir_from_zip(archive: &mut ZipArchive<File>, root: &str, dest: &Path) -> Result<()> {
    for i in 0..archive.len() {
...
static COMPOUND_CACHE: std::sync::OnceLock<
...
fn compound_cache_key(file_path: &Path) -> Result<String> {
    ...
    Ok(format!(
        "{}|{}|{}",
        file_path.canonicalize()?.to_string_lossy(),
        size,
        mtime
    ))
}
...
fn prune_compound_cache_locked(map: &mut std::collections::HashMap<String, CacheEntry>) {
    ...
    let mut items: Vec<(String, SystemTime)> =
...
fn get_world_info_from_dir(dir: &Path) -> Result<PackagePreview> {
    ...
    debug!("Read world dir preview: dir={:?}", dir);
    ...
    if name.is_empty() {
        let level_name_path = dir.join("levelname.txt");
        if level_name_path.exists() {
            if let Some(level_name) = decode_text_bytes(&fs::read(level_name_path)?) {
                let trimmed = level_name.trim();
                if !trimmed.is_empty() {
    ...
    for icon_name in ["world_icon.jpeg", "world_icon.jpg", "world_icon.png"] {
        ...
        match normalize_preview_icon(buf) {
            Ok((normalized_icon, width, height)) => {
                icon_decode = true;
                info!(
                    "World preview icon normalized: world_root={} width={} height={} bytes={}",
                    dir.display(),
                    width,
                    height,
                    normalized_icon.bytes.len()
                );
                icon = Some(normalized_icon);
            }
            Err(error) => {
                warn!(
                    "World preview icon decode failed: world_root={} path={} error={error:?}",
                    dir.display(),
                    icon_path.display()
                );
            }
    ...
    let world_pack_references = world_pack_references_from_dir(dir);
    info!(
        "World preview summary: world_root={} icon_found={} icon_decode={}",
        dir.display(),
        icon_found,
        icon_decode
    );
    debug!(
        "World dir preview resolved: dir={:?}, name={}, icon={}, refs={}, size={}",
        dir,
        name,
        icon.is_some(),
        world_pack_references.len(),
    ...
    Ok(PackagePreview {
        ...
        manifest: None,
        sub_packs: None,
        world_pack_references: (!world_pack_references.is_empty()).then_some(world_pack_references),
        valid: true,
        invalid_reason: None,
    })
}
...
fn read_zip_text_case_insensitive<R: Read + Seek>(
    archive: &mut ZipArchive<R>,
    target_path: &str,
) -> Option<String> {
    ...
    packs
        ...
        .filter(|p| {
            template_roots.iter().all(|root| {
                ...
                if normalized.starts_with(&format!("{}/resource_packs", prefix)) {
                    return false;
                }
                if normalized.starts_with(&format!("{}/behavior_packs", prefix)) {
                    return false;
                }
                true
            })
...
fn find_world_icon_under_root<R: Read + Seek>(
    ...
    for i in 0..archive.len() {
        if let Ok(mut file) = archive.by_index(i) {
            ...
            let name = name.replace('\\', "/").to_ascii_lowercase();
            if !root.is_empty() && !name.starts_with(&root) {
                continue;
            }
...
fn find_any_lang_in_texts<R: Read + Seek>(
    ...
    let base = format!("{}texts/", root_prefix)
        .replace('\\', "/")
        .to_ascii_lowercase();
...
fn get_world_info_from_zip<R: Read + Seek>(
    ...
    debug!("Read world zip preview: root={}", root_prefix);
    ...
    if name.is_empty() {
        let level_name_path = format!("{}levelname.txt", root_prefix);
        if let Some(level_name) = read_zip_text_case_insensitive(archive, &level_name_path) {
            let trimmed = level_name.trim();
            if !trimmed.is_empty() {
                name = trimmed.to_string();
            }
    ...
    if let Some(buf) = find_world_icon_under_root(archive, root_prefix) {
        ...
        match normalize_preview_icon(buf) {
            ...
            Err(error) => {
                warn!(
                    "World preview icon decode failed: world_root={} error={error:?}",
                    root_prefix
                );
            }
        }
    ...
    let world_pack_references = world_pack_references_from_zip(archive, root_prefix);
    info!(
        "World preview summary: world_root={} icon_found={} icon_decode={}",
        root_prefix, icon_found, icon_decode
    );
    debug!(
        "World zip preview resolved: root={}, name={}, icon={}, refs={}",
        root_prefix,
        name,
        icon.is_some(),
        world_pack_references.len()
...
pub fn import_archive_optimized(
    ...
    // World
    if resolve_world_archive_primary(&scan).is_some() || is_world_archive(&scan) {
