...
fn auth_panel(
    ...
    let subtitle = snapshot.profile.as_ref().map_or_else(
        || match snapshot.phase {
            AuthPhase::SignedOut => "登录后可在 Linux 游戏中恢复 Xbox 会话".to_string(),
...
pub(super) fn render_app_chrome(
    ...
    let nav = div()
        ...
        .children(nav_items.into_iter().enumerate().map(|(index, item)| {
            ...
            div()
                .id(SharedString::from(format!("linux-nav-{index}")))
                .relative()
                .w(item_width)
                .h(item_height)
                .rounded(px(17.))
    ...
    div()
        ...
        .left(px(0.))
        .right(px(0.))
        .h(if dialog_open { px(620.) } else { px(60.) })
        .child(topbar)
        .when(dialog_open, |element| {
