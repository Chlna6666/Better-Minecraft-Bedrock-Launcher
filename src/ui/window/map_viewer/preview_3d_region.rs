...
fn legacy_mesh_signature(mesh: &super::preview_3d_legacy::Preview3dMesh) -> LegacyMeshSignature {
    LegacyMeshSignature {
...
fn load_incremental_with_converter(
    loader: impl FnOnce(
        Box<dyn FnMut(Arc<super::preview_3d_legacy::Preview3dMesh>, Preview3dBuildStatus) + Send>,
    ) -> Result<super::preview_3d_legacy::Preview3dMesh, String>,
    ...
    let last = Arc::new(Mutex::new(None::<(LegacyMeshSignature, Preview3dMesh)>));
    let callback_cache = cache.clone();
    let callback_last = last.clone();
    let legacy = loader(Box::new(move |legacy, status| {
        let signature = legacy_mesh_signature(&legacy);
        let converted = convert_legacy_mesh(
            legacy.as_ref().clone(),
            &mut callback_cache
                .lock()
...
pub(super) fn load_preview_3d_mesh_from_mcstructure_blocking(
    ...
    .map(|mesh| convert_legacy_mesh(mesh, &mut RegionMeshReuseCache::default()))
}
...
fn convert_legacy_mesh(
    legacy: super::preview_3d_legacy::Preview3dMesh,
    cache: &mut RegionMeshReuseCache,
) -> Preview3dMesh {
    let started_at = Instant::now();
    let global_center = legacy
        .chunk_meshes
        .first()
        .map(|chunk| {
            [
    ...
    for chunk in &legacy.chunk_meshes {
    ...
    let transparent_faces = chunks
        ...
        .sum::<usize>();
    tracing::debug!(
        processed_chunks = legacy.processed_chunk_count,
        legacy_meshes = legacy.chunk_meshes.len(),
        extracted_faces,
    ...
    Preview3dMesh {
        ...
        min_z: legacy.min_z,
        max_z: legacy.max_z,
        missing_chunks: legacy.missing_chunks,
        chunk_count: legacy.chunk_count,
        processed_chunk_count: legacy.processed_chunk_count,
        subchunk_count: legacy.subchunk_count,
        solid_block_count: legacy.solid_block_count,
        glass_block_count: legacy.glass_block_count,
        water_block_count: legacy.water_block_count,
        lava_block_count: legacy.lava_block_count,
        face_count,
        glass_face_count: transparent_faces,
        water_face_count: 0,
        lava_face_count: 0,
        culled_face_count: legacy.culled_face_count,
        omitted_face_count: legacy.omitted_face_count,
        truncated_chunk_count: legacy.truncated_chunk_count,
        vertex_budget: legacy.vertex_budget,
    }
}
...
fn extract_legacy_faces(
    chunk: &super::preview_3d_legacy::Preview3dChunkMesh,
...
fn pass_for_legacy_range(
    ...
    metadata: Option<&super::preview_3d_legacy::Preview3dFaceMetadata>,
) -> Preview3dRegionPass {
