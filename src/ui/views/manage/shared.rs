...
pub(super) const MANAGE_ASSET_THUMBNAIL_RADIUS_PX: f32 = 5.0;
...
pub(super) fn rounded_asset_thumbnail(
    colors: &ThemeColors,
    icon_path: &SharedString,
    placeholder_icon: SharedString,
    row_background: Hsla,
) -> AnyElement {
    let radius = px(MANAGE_ASSET_THUMBNAIL_RADIUS_PX);
    ...
    root = root.child(
        img(PathBuf::from(icon_path.as_ref()))
            .absolute()
            .inset_0()
            .size_full()
            .rounded(radius)
